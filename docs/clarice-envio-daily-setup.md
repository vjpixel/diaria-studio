...
`reportId` ganha sufixos próprios pra este caminho — `-guard-prereq-fallback-deixou-passar`, `-guard-prereq-fallback-cancelou-{ausente|nao-ok}`, `-guard-prereq-fallback-cancelamento-incompleto-{ausente|nao-ok}`, `-guard-prereq-falhou-sem-pendencia` — nenhum deles é tratado como "ok" por `Diaria-Clarice-Envio-Guard-Alarm`, mesmo quando o fallback "funcionou" (deixou passar OU suspendeu com sucesso): o guard não conseguiu fazer o trabalho FRESCO que existe pra fazer, e isso é sempre digno de atenção do editor. **#5698** — o sufixo final (`ausente` vs `nao-ok`) distingue "nenhum snapshot foi encontrado" (nunca gravado, ou ilegível) de "o freio da noite foi MEDIDO como stop/hold" — as duas ainda suspendem (fail-closed, decisão inalterada), mas a mensagem/`reportId` deixam claro qual evidência motivou a suspensão em vez de colapsar as duas na mesma leitura.
...
**Máquina escolhida pelo editor (260811): `helios`** — mesma máquina do sync das 08:30 (`Diaria-Clarice-Sync`) e do `Diaria-Clarice-Novos` das 09:00 (#5447, antes 11:00 do #5140, antes 17:00), o que elimina o atraso do OneDrive na leitura do store pra este par e mantém as três tasks Clarice na mesma máquina.
