...
**Nome herdado, escopo mudou (#6498, 28/08/2026).** Até o cutover do apex (#467), a home `https://diar.ia.br/` era um tema Beehiiv — daí o nome original `beehiiv-home-meta-check`. Hoje a home é `workers/site/public/index.html`, HTML deste repo. Arquivo(s)/task renomeados nesta mesma unidade (`beehiiv-home-meta-check` → `home-meta-check` em todo o repo); **o rename das UNITS systemd em `300` (`diaria-beehiiv-home-meta-check.{service,timer}` → `diaria-home-meta-check.{service,timer}`) é ação manual pendente do editor** — ver seção "Setup" abaixo.
...
## Rename pendente das units systemd em `300` (#6498, 28/08/2026)

Esta unidade renomeou script(s)/lib/testes/task (`Diaria-Beehiiv-Home-Meta-Check` → `Diaria-Home-Meta-Check`) — mas a task já estava **armada e ativa** em `300` sob o nome antigo (`diaria-beehiiv-home-meta-check.{service,timer}`, confirmado ativo 260812/#5091). `unitBaseName` deriva o nome do unit do NOME da task (`scripts/lib/systemd-units.ts`), então depois deste merge o unit gerado por `setup-systemd-timers.ts --task Diaria-Home-Meta-Check` é `diaria-home-meta-check.{service,timer}` — um par NOVO, não uma atualização do par antigo. **Ação manual pendente do editor em `300`** (mesma classe cat. B do `/diaria-develop` — exige a máquina/systemd real, fora do que esta unidade de worktree pode tocar):

```bash
# no 300, depois do merge (git pull no checkout que serve a task):
