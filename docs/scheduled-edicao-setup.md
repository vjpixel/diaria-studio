...
4. Senão, roda `scripts/sync-code.ts` uma vez (fail-soft) e então invoca, **um por stage**:
   `claude --print --permission-mode acceptEdits --max-turns 120 --output-format json --no-session-persistence /diaria-{N}-{skill} {AAMMDD}`
   para cada entrada de `STAGE_PLAN` (1 pesquisa → 2 escrita → 3 imagens → 4 revisão). Stage cujo sentinela `_internal/.step-N-done.json` já existe é pulado sem spawnar; falha em um stage interrompe o laço e o log registra `stage=N`. **`--output-format json`, não `text` (#8560).** O objeto único que o CLI devolve carrega `total_cost_usd`/`usage`/`modelUsage` — usage REAL medido pelo próprio processo, que `scripts/lib/edition-stage-runner.ts` grava em `stage-status.json` (`session_filter: "cli_json"`) logo após cada stage terminar. Existe porque `--no-session-persistence` garante que NENHUM transcript local é escrito, então `capture-stage-usage.ts` (que lê `~/.claude/projects/`) nunca teria dado a capturar aqui, com ou sem `--session-id` explícito — ver `scripts/lib/cli-usage-json.ts`.
