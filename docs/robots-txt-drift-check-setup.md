...
Isso registra a task `Diaria-Robots-Txt-Drift-Check` (diária, 10:15 — #5113, mudou de "a cada 6h": o conserto é ação manual de dashboard do editor de manhã). Idempotente — re-executar regenera os units. Remover: `systemctl --user disable --now diaria-robots-txt-drift-check.timer`.
