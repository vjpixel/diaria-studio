...
**R3 — Falha de rede ≠ dado ausente.** Nunca colapsar os dois num texto genérico. `rodada.js` (`fetchFailed` vs `found === false`) e `triagem.js` ("gh falhou, mostrando cache de HH:MM" vs "gh falhou e não há cache") são a referência; `apoios.js`/`integracoes.js` ainda colapsam. Erro sempre com caminho de saída: retry, ou instrução do que rodar no terminal — nunca beco sem saída (heurística NN/G #9).
...
**R7 — Estado que espera o editor sempre aponta a ação.** Card de gate pendente sem instrução é beco sem saída — desde a remoção do chat do Studio (#6942), a ação real mora sempre no terminal (`/diaria-4-revisao`/`/diaria-6-agendamento`); o card diz isso explicitamente, nunca só "pendente". Botão desabilitado sempre com o motivo em **texto visível** ao lado, não só `title=` — tooltip não existe em touch (vale pro "Disparar esta onda" da triagem e pro item "Revisão" da nav).

**R8 — Gate humano nunca fica invisível.** Card por gate + push por e-mail quando o gate abre (`studio-push-notify.ts`, `gatesPending`). Sem timeout por design (semântica do `AskUserQuestion` preservada no terminal, #3557). Hidratação de gates pendentes que falha não pode falhar em silêncio — avisar e re-tentar.
...
**R12 — Fluxos de decisão são mobile-first.** Aprovar gate do celular era o caso de uso âncora do epic #3554 — desde a remoção do chat do Studio (#6942), aprovar gate só é possível no terminal (risco aceito explicitamente pelo editor); o que resta mobile-first no painel é a REVISÃO (ler o resumo, o conteúdo, os lints antes de ir pro terminal aprovar). Alvo de toque ≥44px (WCAG 2.5.5/HIG; 24px é o piso AA) com ≥8px entre ações adjacentes. Formulários de decisão sem digitação onde der (opções clicáveis; campo livre é fallback).

**R13 — Widgets seguem o padrão WAI-ARIA correspondente, com enhancement sobre base funcional.** Lição do #2622: a navegação nunca depende de JS carregar — base funcional primeiro, e a camada de enhancement sincroniza URL-hash ↔ estado (deep-link) + `aria-selected`. Abas com `role="tab"` + `aria-selected` gerenciado + setas (APG); erros com `role="alert"`; regiões que atualizam via SSE com `aria-live="polite"` (`assertive` só para erro urgente — abuso dessensibiliza); toggles com `aria-expanded` (`nav.js` faz certo); modais com `<dialog>` nativo (foco, backdrop e Esc de graça — padrão apoios).
...
**R15 — Secret nunca ecoa.** Campo sensível vira `type="password"` e é limpo da tela após envio **mesmo em falha**; telas de diagnóstico mostram presença/ausência e nome da env var, nunca o valor (padrão integrações). Nada de secret em log, plan.json ou URL.

## Notificação e tema

**R16 — Notificação em escada: badge < card < push.** Push (e-mail via Gmail, #3564 — canal padronizado em #5341, decisão do editor de não exigir um app de mensagens novo) só para "espera o editor": gate pendente, halt, CI vermelho persistente — com dedup (1 por gate; janela de 15min do halt banner) e deep-link. Threshold alinhado a impacto, não a evento cru; alerta sem próximo passo é ruído (Datadog, alarm fatigue). A mensagem declara quando o deep-link só resolve na rede local (limitação real do `resolveStudioPublicBaseUrl` hoje). Ressalva aceita conscientemente desde #5341: e-mail não tem a latência nem a interrupção de um push de celular — depende do editor ter notificação de e-mail ativa no telefone.
