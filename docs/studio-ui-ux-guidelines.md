...
**R17 — Dark mode: escurecer o chrome antes de inverter o conteúdo.** A paleta do DS é única e clara por decisão (#4 cores, hierarquia por tamanho/peso — `design-tokens.ts`). Implementado em #3876 (`tokens-css.ts` emite `@media (prefers-color-scheme: dark)` sobrescrevendo `--paper`/`--paper-alt`/`--ink`/`--on-ink` — cobre de graça todo CSS que só consome as custom properties — + `--status-ok`/`--status-danger`/`--status-info` recalibrados pro fundo escuro), seguindo a lição do e-mail (`newsletter-styles.ts`): escurece o chrome/canvas ao redor do conteúdo, não cada elemento — inversão total quebra contraste de blocos pensados em tom claro. Superfícies deliberadamente FORA (preview de e-mail sempre `#fff`, porque e-mail é branco por design — já documentado in-place em `revisao.css`; os 2 dashboards embutidos dos Workers, `diaria-dashboard`/`brevo-dashboard`, ficam fase 2 — CSS inline próprio, não passa por `tokens-css.ts`). Continua valendo: nenhum hardcode de cor fora dos tokens sem comentário justificando.
