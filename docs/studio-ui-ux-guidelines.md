...
**R17 — Dark mode: escurecer o chrome antes de inverter o conteúdo.** A paleta do DS é única e clara por decisão (#4 cores, hierarquia por tamanho/peso — `design-tokens.ts`). Quando o Studio ganhar dark mode (registrado em #3874), seguir a lição do e-mail (`newsletter-styles.ts`): escurece o canvas ao redor do conteúdo, não cada elemento — inversão total quebra contraste de blocos pensados em tom claro. Até lá: nenhum hardcode de cor fora dos tokens sem comentário justificando (ex. legítimo: preview de e-mail sempre `#fff`, porque e-mail é branco por design — documentar in-place).
