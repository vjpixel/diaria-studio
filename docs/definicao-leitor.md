...
**Default mudou pra dedup CANÔNICO cross-plataforma (#7204, #7458 — muda o número publicado).** `event.edicao` guarda o id nativo por plataforma (post id da Beehiiv, broadcast id do Kit, campaignId da Brevo) — com as 3 plataformas no store (desde a #7199), a mesma edição do dia contava até 3× pra quem existe em mais de uma. O CLI agora deduplica por padrão pela chave canônica `AAMMDD` (`scripts/lib/diaria-subscribers-edicao-canonica.ts`, heurística MIN(ts) entre `delivered`/`sent`). `--legacy-per-platform-sum` restaura o comportamento antigo (soma por-plataforma, sem dedup) — só pra comparação/depuração, nunca é o número a citar. Medido ao vivo contra a base real na migração do #7458: `leitores_v1` 220 (legacy) → 221 (canônico) — a diferença não é monotônica (some 1 aqui, aparece 1 ali) porque deduplicar cliques encolhe o denominador do CTR, o que pode empurrar um leitor de fronteira pra dentro OU pra fora do piso, independente do corte de recebidas.
