...
| (v) | `hermes/scripts/watch-continuo-health.sh` | Fora deste repo — cron do Hermes no `300`, `--no-agent`, 1x/dia | Escopo estreito: SÓ `continuo` — review Opus diário atrasado, `failure_streak`, claim vazado (heartbeat parado >45min com claim ativo), vazamento pago | 26h (review), 2 (streak), 45min (claim), thresholds próprios do script |
