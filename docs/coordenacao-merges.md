...
2. A coordenadora **verifica por conta própria** — nunca só pelo relato: gate de autenticidade de review (`scripts/check-pr-review-authenticity.ts --pr N`), CI, mergeabilidade (`gh api pulls/N` → `mergeable_state`), e que o head reportado é o head atual.
3. Mergeia **uma por vez**, com o merge lock (`session-registry.ts merge-lock-acquire --pr N` → `gh pr merge` → `merge-lock-release --pr N`), e devolve o **SHA em master** a quem sinalizou.
