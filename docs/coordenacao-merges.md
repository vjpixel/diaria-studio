...
1b. **A coordenadora varre `gh pr list --state open` periodicamente — o sinal do passo 1 nunca é o único caminho.** Sinalizar depende de a outra sessão estar viva e desocupada na hora certa, e é justamente isso que falha: em 09/09/2026, com o protocolo já em vigor, as PRs #7749 e #7754 (mesmo fix do #7743, escritas por sessões diferentes com 24 min de diferença) chegaram à coordenadora só porque ela varreu a lista por conta própria — nenhuma das duas foi sinalizada, e a duplicação que a seção **Motivação** descreve se repetiu. A varredura é um comando, não depende de disciplina alheia, e é o que pega PR órfã, PR de sessão que terminou antes de sinalizar e PR duplicada em voo. O sinal continua valendo pelo que a varredura não vê: contexto de review, findings tratados, prioridade.
