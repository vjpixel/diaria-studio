...
5a. **MCP `doppler` (opcional, #8676) — consulta interativa de secrets dentro da sessão.** Distinto do `doppler run` (injeção de env vars em processos automatizados, inalterado) — este MCP é pra buscar/conferir um secret sem sair da sessão. `.mcp.json` já declara o server (`npx -y @dopplerhq/mcp-server --read-only`, autenticando via `${DOPPLER_MCP_TOKEN}`). Gerar um **Service Account Token dedicado, escopo SOMENTE LEITURA** no painel do Doppler (workspace `diar.ia.br` → Access → Service Tokens) — **não reusar** o token que autentica `doppler login`/`doppler run` nesta máquina, que tem escopo mais amplo do que uma sessão interativa precisa (princípio de menor privilégio; o pedido original da #8676 já sinalizava isso). Exportar como `DOPPLER_MCP_TOKEN` (mesmo mecanismo do passo 1 acima, por variável de ambiente — nunca literal em `.mcp.json` nem em arquivo versionado). O `--read-only` no comando bloqueia os write tools no client, mas é defesa em profundidade — o escopo do token é a defesa real (docs.doppler.com/docs/mcp confirma: "all token permissions are ultimately validated by the Doppler API"). Confirmar com `/mcp`.
