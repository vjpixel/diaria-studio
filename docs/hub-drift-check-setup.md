...
Isso registra a task `Diaria-Hub-Drift-Check` (diária, 10:00 — #5113, mudou de "a cada 6h": o conserto é ação manual do editor de manhã). Idempotente — re-executar regenera os units. Remover: `systemctl --user disable --now diaria-hub-drift-check.timer`.
