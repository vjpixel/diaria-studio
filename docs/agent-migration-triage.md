...
| `scorer` | opus 4.6 → Sonnet (#2772, 260701) | 1×/edição | **Manter** | Score 0-100 com raciocínio editorial sobre relevância pra audiência + diversidade entre destaques. Não é função pura — depende de `audience-profile.md` + contexto da edição. Heurísticas (-X paywall, +Y Brasil) já existem em `scripts/lib/` como helpers, mas a decisão final é editorial. |
| `writer` | sonnet 4.6 | 1×/edição | **Manter** | Geração de newsletter editorial. Irredutivelmente criativo. |
| `social-linkedin` | sonnet 4.6 | 1×/edição | ~~Manter~~ **Removido (#7120)** | A justificativa original ("6 textos auxiliares/comments por edição") já tinha ficado obsoleta com o #3627 (260716, aposentou `comment_diaria`/`comment_pixel`); o agente foi depois colapsado em `social-writer` pelo #3991 (260724) — não era mais dispatchado no Stage 2 — e o arquivo removido do repo. |
| `social-facebook` | sonnet 4.6 | 1×/edição | ~~Manter~~ **Removido (#7120)** | Idem `social-linkedin`. |
| `title-picker` | opus 4.7 → Sonnet (#2772, 260701) | fallback 1×/edição | **Manter** | Escolha 1-de-3 títulos com raciocínio sobre concretude + CTR. Opus deliberado por alto impacto editorial (#159) — revisto em #2772: escolha entre opções já escritas é mais mecânica que julgamento holístico, migrado pra Sonnet. |
...
| `social-instagram` | haiku | n/a (não usado) | ~~Remover~~ **FEITO (#1846), depois recriado e removido de novo (#7120)** | Correção: os arquivos de agente NÃO tinham deixado de existir em #1846 — o texto anterior estava errado. O agente foi recriado pelo #3486 (diferenciação de texto por canal), depois colapsado em `social-writer` pelo #3991 (260724) e finalmente removido do repo pelo #7120. |
