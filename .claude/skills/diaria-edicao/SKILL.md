...
**Condição pra rodar:** `npx tsx -e "import { shouldRunRepliesAtTopLevel } from './scripts/lib/replies-top-level-gate.ts'; console.log(shouldRunRepliesAtTopLevel({no-gates foi passado a esta invocação de /diaria-edicao}))"` — ou, equivalente, direto: roda **sempre que `--no-gates` NÃO foi passado** à invocação original de `/diaria-edicao` (editor presente). Com `--no-gates`: pular a seção inteira, sem log (sessão desassistida — mesma semântica de sempre para `pre_gate`).
...
**Se Gmail MCP estiver indisponível** (probe falhou, ou já é sabido offline desta sessão): pular a seção — mas **sempre** logar o skip primeiro, nunca em silêncio (#7166 item B — a causa da regressão de 2 semanas foi justamente um skip que não logou), com o comando exato de `npx tsx -e "import { buildRepliesSkipLogArgs } from './scripts/lib/replies-skip-log.ts'; console.log(buildRepliesSkipLogArgs('gmail_mcp_unavailable', '{AAMMDD}').join(' '))"` (rodar o comando resultante). Idem quando a condição do parágrafo anterior for `false` (`--no-gates` passado), usando `'no_editor_supervision'` no lugar de `'gmail_mcp_unavailable'`.
