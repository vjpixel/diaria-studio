...
7. **Plano da rodada**: gravar em `data/overnight/{AAMMDD}/plan.json`. **`{AAMMDD}` = data local de início da rodada, fixada AQUI e relida de `plan.json` em todas as fases seguintes — nunca recomputada de today()** (toda rodada cruza a meia-noite).
   ```json
   {
     ...
     "machine_id": "hostname desta máquina",
   ...
   ```
   **`machine_id` (#3033, adotado pra overnight em #3322).** `data/` é um junction do OneDrive sincronizado entre máquinas — o `plan.json` desta rodada fica visível pra QUALQUER outra máquina no mesmo OneDrive, e vice-versa. Gravar o campo `machine_id` no nível raiz com o output de `npx tsx scripts/lib/machine-id.ts` (hostname desta máquina) no momento da criação do plano (este passo); não precisa reconsultar a cada re-write subsequente. Sem esse campo, o hook `pr-create-review.mjs` (guard de sessão ativa, #3322) e a statusLine de outra máquina no mesmo OneDrive podem confundir a rodada desta máquina com uma rodada de outra (`isForeignDevelopPlan` em `scripts/overnight-statusline.ts` filtra por esse campo; ausente = tratado como legado, fail-open, não filtrado).
...
2. **Dispatchar um subagente implementador por unidade de trabalho** — issue solo ou lote inteiro (#2024) — (`Agent`, `subagent_type: "general-purpose"`, `isolation: "worktree"` — nunca um agente especializado do repo: eles têm toolset restrito e não conseguem commitar/pushar). O prompt do subagente inclui, obrigatoriamente:
   ...
   - **convenção de branch (#3321) — instrução literal, não implícita**: "nomeie a branch como `overnight/fix-{issue}-{slug}`" (issue solo) ou `overnight/batch-{slug}` (lote). O prefixo `overnight/` é **obrigatório**, não estético: o hook `.claude/hooks/pr-create-review.mjs` (`resolveEffort`) usa esse prefixo pra decidir `low` vs `max` effort no `/code-review` pós-`gh pr create`, e o cleanup de fim de rodada (Fase 1 passo 3) assume esse padrão pra deletar a branch remota. **Causa raiz do incidente 260710 (#3321)**: essa convenção estava documentada mais abaixo neste mesmo passo (parágrafo narrativo, não nesta lista "obrigatoriamente") e nenhum PR da rodada a seguiu — todo PR pagou o fleet `max` completo (10 finders + verify + sweep) em vez do `low` token-otimizado, o maior contribuinte mecânico único pro gasto de tokens da rodada. Retry defensivo: se o PR aberto pelo subagente não tiver o prefixo, o coordenador renomeia a branch antes do passo 3 (`git branch -m` local + push da branch renomeada + delete da antiga) — não silenciosamente aceitar o naming errado;
