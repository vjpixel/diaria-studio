...
3. **Varredura**: `gh issue list --state open --limit 200 --json number,title,labels,body,url`. **Com `--bugs` (#3375)**: filtrar o resultado (ou usar `--label bug` direto no comando) para só issues com label `bug` — as demais nem entram na classificação do passo 4, não precisam de comentário/skip explícito (não são "puladas por bloqueio", estão simplesmente fora de escopo do modo). Registrar em `plan.json` (campo `bugs_only: true`) e no relatório final quantas issues foram excluídas por esse motivo, pra visibilidade.
