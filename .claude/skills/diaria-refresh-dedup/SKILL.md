...
description: (Opcional) Regenera manualmente `data/past-editions.md`. Normalmente não é necessário — `/diaria-1-pesquisa` e `/diaria-edicao` já fazem isso automaticamente no Stage 0. Útil para forçar refresh fora do fluxo ou inspecionar o diff.
---
...
- Inspecionar o conteúdo do `data/past-editions.md` fora do fluxo de edição.
...
- **Bootstrap** (primeira vez): busca as `dedupEditionCount` edições mais recentes e popula `data/past-editions-raw.json` + `data/past-editions.md` do zero.
...
Ao final, mostre ao usuário o JSON de resultado (`{ mode, new_posts, total_in_base, most_recent_date, skipped: false, md_regenerated: true }`) e, se `new_posts > 0`, as primeiras linhas de `data/past-editions.md` para confirmação visual.
