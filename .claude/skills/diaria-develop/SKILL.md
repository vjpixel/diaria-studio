...
**Gate de drift de label de decisão (#5892), logo em seguida:** rodar `npx tsx scripts/check-decision-label-drift-gate.ts --plan data/develop/{AAMMDD}/plan.json`. Mesmo script do overnight — detecta drift entre a prosa da sessão e as labels estruturais atuais (racional completo, incluindo as duas fontes de prosa do #5955 e o procedimento de resolução: `context/overnight-dispatch-rules.md` item 24). `exit 0` (nenhum drift) → seguir pro relatório. `exit 1` (drift detectado) → aplicar as labels estruturais faltantes reportadas (procedimento no item 24 citado acima) antes de continuar — o relatório NÃO compila enquanto houver achados não resolvidos.
