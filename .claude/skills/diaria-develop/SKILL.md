...
- **"Sempre" é literal**: se uma re-checagem entre ondas trouxer um bug novo e a sessão voltar pro grupo 1a, ao retornar ao grupo 3 o gate pergunta **de novo** — o backlog é outro, a resposta anterior não é necessariamente a mesma. Registrar cada disparo em `goal.tier3_gate.asked_at` (lista, não escalar).
...
9. Gravar `plan.json`, incluindo o campo `goal` inicial — `tiers` = `{ "1a": [...], "1b": [...], "2": [...], "3": [...] }` (números das issues em cada onda, já filtrados por `--issues`/`--only`/`--bugs`/`--priority`), `current_tier: "1a"`, `policy` ainda `null` (coletada na Fase 0.5 — exceto com `--no-implement`, que grava `table_only` direto), `target_set`/`remaining` **ainda vazios** (só populados quando a Fase 0.5 souber a política — `blocked_only` usa só 1a-bloqueada+1b, `exhaust_all` usa as 4 ondas inteiras), `reached: false`, `rescans_done: 0`, `tier3_gate: { "asked_at": [], "decision": null, "selected": null }`. Com `--dry-run`, **parar aqui**.
...
Assim que `goal_policy` é respondida, popular `goal.target_set`/`goal.remaining` no `plan.json`: com `exhaust_all`, é a união das 4 ondas (`tiers["1a"] + tiers["1b"] + tiers["2"] + tiers["3"]`); com `blocked_only`, é só `tiers["1a"]` filtrada pra bloqueadas + `tiers["1b"]` (o mesmo conjunto de sempre, só que lido a partir da partição por onda em vez de recalculado); com `table_only`, `target_set`/`remaining` ficam vazios (não avaliados). `reached` recalcula pra `target_set.length === 0` no mesmo passo.
...
A maioria dos desbloqueios já foi **coletada no briefing FRONT-LOADED** (Fase 0.5, #2966): a Fase 1 processa as issues `desbloqueada-validada` direto na implementação, e só usa o **Gate 1 pro RESÍDUO** — o que não deu pra front-load (token que o editor foi gerar, ambiguidade imprevista). O Gate 1 remanescente é serial mas **agrupa desbloqueios da mesma categoria numa única `AskUserQuestion`** (até 4 perguntas). Trabalho acontece **um tier por vez** (`goal.current_tier`, começa em `1a`) — só issues do tier corrente entram na fila de implementação; um tier só é aberto pra trabalho depois que o anterior está esgotado (todo status terminal). **No início de cada iteração, reler `plan.json`.**
...
6. **Esgotar o tier corrente → avançar (`goal_policy ∈ {exhaust_all, blocked_only}`):** quando todas as issues do `current_tier` atingem status terminal:
   ...
   - Quando a onda 3 (ou 1b, em `blocked_only`) esgota **sem** issue nova na re-checagem seguinte: rodar a **re-varredura de convergência** (mesma varredura completa, agora perguntando "achou algo novo em qualquer tier?") — repete **sem cap** enquanto achar issue nova sem status terminal; sem novidade, `goal.reached: true` (ou `false` se o gate do grupo 3 decidiu `parar`/`escolher` com resíduo).
...
"goal": {
  ...
  "rescans_done": 3,
  "findings_depth": 1,
  "tier3_gate": { "asked_at": ["2026-07-29T21:40:11Z"], "decision": "entrar", "selected": null }
}
```
...
`tier3_gate` = registro do gate obrigatório antes do grupo 3 (seção própria). `asked_at` é **lista** (o gate pode disparar mais de uma vez na mesma sessão, se a fila voltar pro grupo 1a e o retorno ao grupo 3 depois for uma reentrada — cada disparo é um item novo na lista). `decision` ∈ `entrar`|`parar`|`escolher`|`sem-resposta` (o último grava quando o Fallback de ausência interrompeu no meio do próprio gate). `selected` = array de números escolhidos quando `decision: "escolher"`, `null` nos demais casos. Um gate já respondido **não** é re-perguntado dentro do mesmo ciclo em resume — mas uma nova entrada no grupo 3 depois de uma volta a tier anterior é um gate novo, não um resume do anterior.
