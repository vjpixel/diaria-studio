...
| **C** | decisão-produto/editorial (ex: design system; UX trade-off) | escolhe o trade-off (`AskUserQuestion`) | **posta a decisão como comentário durável** na issue (marcador `decisao-editor`, #5373), remove a ambiguidade (→ elegível) — inclusive **removendo a label `trade-off-real`** se presente (`gh issue edit N --remove-label trade-off-real`, #5462: sem isso a issue fica presa em Develop na Triagem para sempre depois de já decidida) —, implementa a opção escolhida. **Gate mecânico obrigatório logo após postar o comentário (#5821):** `npx tsx scripts/check-trade-off-label-cleared.ts --issue N` — sai `exit 1` se a decisão foi registrada mas a label ainda está presente (sinal de que o `--remove-label` acima foi esquecido); rodar o comando de remoção e checar de novo antes de seguir pra implementação. Antes da #5821, essa checagem era só prosa neste item da tabela — o esquecimento (#5415) não era pego mecanicamente | a decisão postada como comentário **é** a evidência durável, mais o `exit 0` de `check-trade-off-label-cleared.ts` como confirmação de que a label foi de fato removida |
...
- **Re-checagem ao fim de CADA unidade (#5907 item d)** — além dos dois momentos abaixo, o `gh issue list` barato roda ao fechar cada unidade (merge ou status terminal), não só quando a onda/tier esgota. Motivo real da 260821c: a label `trade-off-real` da #5891 entrou às 22:37Z, DEPOIS da única varredura da Fase 0 e fora de janela de re-checagem — a issue ficou invisível pra sessão inteira até a convergência da Fase 2, já com `deixado-para-o-helios` gravado. Label/classificação muda mid-sessão por ação de OUTRA sessão (overnight paralelo, editor no Studio); esperar o fim do tier é dar à mudança uma janela de horas de cegueira evitável por um comando de ~1s. O resultado alimenta a mesma reclassificação da re-checagem entre ondas (issue que virou develop/bloqueada/agendada sai da fila corrente; novata entra pela onda certa).
