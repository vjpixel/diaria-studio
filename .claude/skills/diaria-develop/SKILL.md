...
Só entram na checagem issues que ESTA sessão **terminou de trabalhar** (`mergeada`, `entregue-fora-de-codigo`, ou `pulada` motivo `ja-resolvida-antes-da-sessao`). Issue `nao-tentada` ou `deixado-para-o-helios` **nunca é acusada** — ela continua sendo fila legítima do Develop, e é exatamente o caso das 2 issues corretas da medição (#6048, #467). O gate **não decide o track de destino**: ele detecta o resíduo e para; qual é o track verdadeiro é julgamento da sessão, registrado via `route-issue` (mesmo princípio do #5373 — julgamento feito uma vez por quem tem contexto, gravado, nunca re-derivado por heurística).
