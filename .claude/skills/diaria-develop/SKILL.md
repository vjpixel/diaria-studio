...
6. **Teto de concorrência por sessão** — PARCIALMENTE MECANIZADO (#5161 fleet review item 7): read-path mecanizado (`session-registry.ts list-active` soma `active_worktrees` de outras sessões antes de decidir quantos worktrees abrir nesta onda, ver "Paralelização segura" e Fase 1 passo 4 acima); write-path (`heartbeat --active-worktrees N`) **ainda não é chamado por ESTA skill** — nenhum call site do `/diaria-develop` invoca esse comando conforme worktrees abrem/fecham, então o `active_worktrees` DESTA sessão fica sempre ausente na prática até isso ser implementado. **O `/diaria-overnight` passou a chamá-lo desde o #6299** (dois call sites obrigatórios: ao dispatchar a onda e após o cleanup de cada worktree) — antes disso nenhuma das duas chamava, e esta linha dizia "nenhuma skill", o que deixou de ser verdade. A assimetria resultante é conservadora na direção certa (quem publica é quem tem o teto menor: overnight, 3), mas **não é simetria** — a heurística do passo 4 desta skill soma zero pros worktrees do overnight nunca, e o overnight soma zero pros desta skill sempre. Fechar este lado é trabalho pendente, não pressuposto: enquanto não fechar, duas sessões concorrentes podem somar até 9 worktrees na máquina sem nenhuma das duas enxergar.
