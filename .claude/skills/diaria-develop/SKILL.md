...
**Por padrão (#4297), a sessão tenta esgotar o backlog bloqueado do escopo antes de encerrar** — mesma propriedade de esgotamento do overnight ("resolve a fila autonomamente até esgotá-la"), aplicada ao conjunto complementar. Não é uma garantia incondicional: o Fallback de ausência (editor sai no meio), o cap de re-scan (`rescans_done ≥ 2`) e o guard de colisão editorial continuam podendo encerrar a sessão com o alvo não esgotado — nesse caso o Goal fica registrado como não atingido, não bloqueia o encerramento. Ver seção "Goal de esgotamento" abaixo pra definição exata do que conta como alvo e como terminal; `goal_policy = table_only` no briefing (ou `--no-implement`, que força o mesmo comportamento) volta ao comportamento antigo (fecha quando a tabela da Fase 0 acaba, sem re-varredura).
...
`--issues`, `--only`, `--bugs` (#3375) e `--priority` (#3499) restringem o conjunto-alvo. O Goal é avaliado **contra o alvo já filtrado**, nunca contra o backlog bloqueado total do repositório — senão qualquer sessão `--bugs` ou `--priority P0` fica com Goal permanentemente inatingível. O relatório da Fase 2 sempre diz qual escopo está sendo avaliado: `Goal (escopo: --bugs --priority P0,P1): atingido`.
...
**Incompatibilidade estrutural com `--no-implement`:** nesse modo, TODA issue do escopo termina em `unblock_status: desbloqueada-validada` + `status: pendente` por desenho (implementação fica pro overnight seguinte) — nenhum dos 4 terminais da subseção anterior é alcançável, porque implementar é justamente o passo que produz `mergeada`/`draft-ci-vermelho`. Com `goal_policy: exhaust` (default) isso faria a sessão gastar os 2 re-scans e fechar reportando `Goal: não atingido` mesmo tendo feito exatamente o que `--no-implement` pediu. Por isso, **`--no-implement` força a avaliação do Goal a se comportar como `table_only`** (sem re-varredura, sem tentar atingir `reached`) — a Fase 2 reporta `Goal: não avaliado (--no-implement ativo)`, não "não atingido". A pergunta de `goal_policy` no briefing (item 7 da Fase 0.5) é pulada quando `--no-implement` está ativo — não há política real pra escolher.

### O limite que NÃO pode ser cruzado

O develop é supervisionado **por natureza**. O Goal **não** revoga o "Fallback de ausência" (ver seção Gates): se o editor sai, a issue corrente vira `pulada` motivo `decisao-adiada`, e **antes** de gravar `resume_state` e parar, recalcular `goal.remaining`/`goal.reached` com esse status já aplicado (mesmo recompute do passo 5 da Fase 1) — **a sessão para limpa — nunca continua autônoma**. Essa atualização acontece independente de a Fase 2 rodar ou não nesta invocação (uma saída abrupta pode interromper antes da Fase 2 — o `plan.json` gravado é que precisa estar correto pro resume ou pro próximo relatório, não uma promessa de que o relatório sai agora). Se o editor sair com o alvo não esgotado, o Goal fica registrado como **não atingido** com o resíduo no handoff da Fase 2 (quando ela rodar), pra ser retomado no próximo `/diaria-develop`. O Goal também **não** transforma os gates humanos (Gate 1, Gate de Onda, Gate B) em automáticos — é critério de completude, não licença pra rodar sem o editor.

## Fase 0 — Montar e triar a fila BLOQUEADA (filtro invertido)

0. **Resume** via `plan.json` se existe.
1. **Sync:** `git checkout master && git pull`; capturar `base_sha = git rev-parse HEAD`; **capturar `started_at` = timestamp ISO 8601 REAL desta sessão AGORA** (`date -u +%Y-%m-%dT%H:%M:%SZ`, ou equivalente) — gravado em `started_at` no `plan.json` do passo 7 abaixo. **Nunca gravar a string `{AAMMDD}` nesse campo** (#3841 — mesmo bug de raiz do overnight: `{AAMMDD}` nomeia só o diretório da sessão, não é uma data parseável; `started_at` precisa ser um ISO real pro painel `/rodada` do Studio conseguir ordenar TODAS as rodadas — overnight e develop — numa sequência cronológica única). `gh auth status`. **Log de startup do modelo/effort do coordenador (#3454, espelha #2993 do overnight):** logar no run-log o modelo/effort **CONFIGURADO** pelo frontmatter desta skill (`sonnet` / `high`) — não o auto-relatado —, pra tornar o pin verificável via `/diaria-log {AAMMDD}`:
   ```bash
...
4. **Classificar** cada bloqueio em A–E. Issue herdada só por `requer-sessao-local` (sem nenhuma label de categoria A–E aplicável) não força uma categoria — listar na tabela do passo 5 com categoria `local` em vez de A-E (mesmo tratamento informacional da seção "Label `local`").
...
7. Gravar `plan.json`, incluindo o campo `goal` inicial — `target_set` = os números das issues classificadas como "entra no alvo" (tabela da seção "Goal de esgotamento", já filtradas por `--issues`/`--only`/`--bugs`/`--priority`), `policy` ainda `null` (coletada na Fase 0.5 — exceto com `--no-implement`, que grava `table_only` direto), `reached: target_set.length === 0` (não hardcodar `false` — um escopo filtrado que já não tem nenhuma issue-alvo nasce com o Goal atingido, não pendente), `remaining` = cópia de `target_set`, `rescans_done: 0`. Com `--dry-run`, **parar aqui**.
...
**Abrir sempre com o status do Goal (#4297)**, antes de qualquer outra linha: `Goal (escopo: {descrição do escopo efetivo}): atingido` ou `não atingido`. Escopo efetivo = `--issues`/`--only`/`--bugs`/`--priority` da invocação, resumidos (ex: `--bugs --priority P0,P1`; `nenhum filtro` se a sessão rodou sem flags de escopo). Com `goal.policy: "table_only"`, a linha vira `Goal: não avaliado (goal_policy=table_only)` — não é "não atingido", é fora de escopo por opção do editor no briefing. Com `--no-implement` ativo, a linha vira `Goal: não avaliado (--no-implement ativo)` pelo mesmo motivo (ver "Incompatibilidade estrutural com `--no-implement`"). Quando não atingido, listar os `goal.remaining` com o motivo POR ISSUE de cada uma não ter chegado a terminal (bloqueio cat. B ainda sem conta de terceiro, token cat. A não colado, etc. — consultado no registro por-issue do `plan.json`, `goal.remaining` guarda só os números). Separado disso, se a sessão encerrou porque `goal.rescans_done` bateu o cap `2` (motivo de nível de SESSÃO, não de uma issue específica), dizer isso explicitamente numa frase à parte. É o resíduo que a Seção de HANDOFF abaixo também referencia.
