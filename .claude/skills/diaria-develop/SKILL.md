...
| **Paralelização** | **também paraleliza desde o #6299** — mesma análise de cluster, teto **6** desde o #8486 (era 3; igual ao do develop — ver a seção de onda no overnight). Só o MERGE é serial | **paraleliza tudo que for seguro**; issues que colidem em arquivo **fundem numa unidade só** (1 PR, `Closes` múltiplo) em vez de serializar em ondas (#4319) — teto de 6 worktrees |
...
## Regra permanente: sessão interativa não faz o que o `300` faria sozinho (#5751)

Pedido direto do editor, 20/08/2026: "tudo que puder deixar para o `300`
fazer e não for atrapalhar seu trabalho, deixe." O `/diaria-overnight` roda
desassistido no servidor `300` e drena o backlog desbloqueado por conta
...
é trabalho duplicado, e o modo de falha é real: uma rodada `300` teve
...
sessões atacando o mesmo trabalho que uma só (o `300`) resolveria de
...
3. **O editor pediu isto nesta conversa** — pedido direto tem precedência
   ...
   direto). **(#7946) Esse pedido também vence o MOTIVO do bloqueio, não só
   a competição com o `300`** — ver "Escopo do Gate 1 residual" (cat. A/B
   sem front-load) e a nota após a tabela "Categorias de bloqueio" (bloqueio
   sem categoria A-E mapeada, ex: veto de priorização por funil da whitelist
   AAARRR) na Fase 1.
...
grava quando a issue exige editor presente (`windows`, `develop-track`,
bloqueio que o editor destrava ao vivo — `trade-off-real` saiu desta lista
no #7493: virou pergunta de briefing do overnight, não veredito `develop`), e é o que a Fase 0 passo 6a grava
em `exec_track_painel`. Simetria: **o default "fica para o `300`"
...
filtro como "elegível pro 300", e `validate-develop-plan-motivo.ts`
(#5914, gap b) recusa `deixado-para-o-300` nesses tracks. Por isso o
...
**Todo o resto fica para o `300`** — mesmo issue elegível (sem bloqueio,
...
`300` o que exige gate humano nem o que exige esta máquina — reduz
...
corrente entrar em Gate 1/implementação (Fase 1, ver "Filtro do servidor 300"
abaixo): decide, issue por issue, se ela é trabalho desta sessão ou se
fica para o `300`. Na prática, sob `exhaust_all`, uma issue elegível
...
**Filtro do servidor 300 — onde roda.** Fase 1, imediatamente antes do passo 1
...
`deixado-para-o-300`, **sem** Gate 1, **sem** worktree, **sem**
comentário durável na issue (diferente de `nao-destravavel-na-sessao`: isto
não é bloqueio, é só "não é esta sessão que trabalha isto" — o `300`
...
`motivo: "deixado-para-o-300"`) e no relatório da Fase 2 como lista
agregada ("N issues elegíveis deixadas para o 300: #A, #B, ...") — não
issue a issue na resposta ao editor. **Se a issue estava reivindicada por
esta sessão (`claim-issue` rodou numa onda anterior e o trabalho não foi
adiante), soltar a claim aqui:** `npx tsx scripts/lib/session-registry.ts
...
contraditório e bloqueia o `300` por até 90min à toa (#6317). No-op
...
sessão trabalha (filtro do `300`); esta decide **por quanto tempo seguido**
...
- **`AAMMDD` (opcional)** — data-rótulo da sessão (nomeia `data/develop/{AAMMDD}/plan.json`). **Não é data de edição** (nenhum stage editorial destrutivo depende dela; a regra D+1 não se aplica). O default é seguro (hoje = data do tick, nome do arquivo em `data/develop/`), **aplicado automaticamente com banner** (`Data assumida: {AAMMDD}. Passe AAMMDD explicitamente para outra sessão.`) — nunca pergunta por confirmação (mesma forma que #5321 impôs ao pipeline: imprimir a data assumida em destaque, com instrução de passar `AAMMDD` para outra edição). A data é fixada no `plan.json` no início da Fase 0 e relida de lá; se a sessão cruzar meia-noite, o arquivo já indica o rótulo correto para retomar.
...
**Correção factual (#6299): não é a supervisão humana que torna o paralelo seguro.** Este parágrafo afirmava isso, e a afirmação sustentava a serialização do overnight — mas ela não se verifica. O que torna o paralelo seguro é um conjunto de mecanismos DETERMINÍSTICOS, todos construídos depois daquela decisão e nenhum deles exigindo o editor presente: worktree isolado por unidade, análise de cluster de conflito, fusão de cluster colidente (#4319), `is-claimed`/`claim-issue` check-and-set (#6236), `conflicts` por arquivo (#6168), merge lock, fleet review pré-merge obrigatório (#4383), Gate 2 determinístico (#2210/#2222) e o guard de `gh pr merge` por subagente (#5716). A supervisão humana cobre outra coisa — o Gate 1 cat. C (trade-off editorial) e o Gate B (blast-radius) —, e é isso, não o paralelismo, que continua separando esta skill do overnight.

**"Seguro" = sem colisão de arquivo entre PRs diferentes**, via análise de **cluster de conflito**:
...
**"Editor faz X" da tabela acima só se aplica no front-load (#5695).** A coluna "cola o token/chave" (cat. A) / "confirma que a conta já existe" (cat. B) descreve o que acontece na Fase 0.5 (item 3/4, batchado, "colar TODOS os tokens/confirmar TODAS as contas de uma vez"). **Não** descreve mais um `AskUserQuestion` por issue no Gate 1 residual da Fase 1 — cat. A/B que chegam lá sem front-load já resolvido seguem por default pro terminal `nao-destravavel-na-sessao` sem novo pedido (ver Fase 1, "Escopo do Gate 1 residual" acima), **exceto quando a issue foi pedida explicitamente via `--issues` nesta invocação (#7946) — ver a exceção lá**. O editor colar o insumo depois disso é sempre iniciativa dele, nunca a skill perguntando de novo. Cat. C mantém `AskUserQuestion` sempre (trade-off editorial real, critério 2 do #5321, sem default seguro possível). Cat. E só pergunta quando algum caminho envolve gasto real (critério 3) — sem sinal de custo no corpo da issue, o default é "documentar" (caminho sempre seguro e reversível: converte pra `elegível-documentada`, nunca aplica um workaround não revisado sem perguntar).

**Bloqueio SEM categoria A-E mapeada (#7946).** Nem todo `bloqueada` de `classifyExecTrack` cai numa das 5 categorias acima — ex: o veto de priorização por funil da whitelist AAARRR (`matched: "label:aarrr-fora-da-whitelist"`, `scripts/lib/aarrr-whitelist.ts`) não é credencial, conta, trade-off, blast-radius nem plataforma; é só uma escolha de ONDE investir esforço de crescimento novo. Sem `--issues` citando a issue, esse bloqueio segue como está — nenhuma sessão autônoma o resolve, por design (é o próprio ponto da whitelist). **Com `--issues N,M` citando a issue nominalmente, o pedido explícito já É a decisão de prioridade** (mesmo racional do critério 3 acima e do princípio "perguntar é exceção" — o editor não está pedindo permissão pra ignorar a whitelist, está exercendo a exceção que só ele pode abrir): a issue entra na fila de resolução como se elegível — sem `AskUserQuestion` (não há trade-off nem insumo faltando pra perguntar, só uma prioridade que o pedido explícito já resolveu) — e o coordenador registra o override como comentário durável na issue (ex: "processada apesar do veto de priorização da whitelist AAARRR, por pedido explícito via `--issues` em {data}") **e imprime um banner no terminal** (`Issue #N — bloqueio de PRIORIZAÇÃO (não técnico) ignorado por pedido explícito via --issues: {matched}.`), já que aqui é uma decisão de política estratégica (a whitelist), não um insumo técnico faltando — merece visibilidade maior que só o comentário na issue. Isto **não** desarma um bloqueio de categoria A-E mapeada coexistindo (credencial ainda ausente continua ausente) — cobre só o motivo SEM categoria, que por definição não depende de nenhum insumo que o `--issues` não supra.
...
**Antes de classificar como cat. C — decisão-produto (#5373):** se a issue tem a label `decisao-registrada` (ou mesmo sem ela, como rede de segurança), rodar `npx tsx scripts/lib/issue-decisions.ts --issue N` (`scripts/lib/issue-decisions.ts`). **Comparação concreta (#6961):** a decisão vale quando é o marcador MAIS RECENTE da thread (comparada contra `execution_block`, nunca contra `updatedAt` da issue — o próprio POST do marcador bumpa `updatedAt`, tornando `decided_at >= updatedAt` insatisfazível por construção). Decisão encontrada e mais recente que um eventual bloqueio → a issue **não** é cat. C — a decisão já existe, usar como contexto e classificar pela EXECUÇÃO restante (elegível se nada mais falta; cat. A/B/E se a execução esbarra em bloqueio novo distinto da decisão em si). Só reavaliar como cat. C se existir um `execution_block` mais recente que a decisão (ver item seguinte).
...
| `ambigua` **quando trade-off-real** (cat. C) | ~~escopo exclusivo do develop por decisão do #2640~~ — **#7493 tirou esta linha do alvo**: a label `trade-off-real` classifica `overnight` e a pergunta é feita no briefing do próprio overnight. Só entra aqui se vier acompanhada de `windows`/`develop-track` (que vencem na precedência) ou se o editor a trouxer nesta sessão |
...
- `deixado-para-o-300` (#5751, 20/08/2026 — issue elegível que não bateu nenhum dos 4 critérios da regra "sessão interativa não faz o que o 300 faria sozinho", filtrada ANTES de chegar em Gate 1/implementação; ver seção própria no topo deste arquivo)
...
- **Re-checagem ao fim de CADA unidade (#5907 item d)** — além dos dois momentos abaixo, o `gh issue list` barato roda ao fechar cada unidade (merge ou status terminal), não só quando a onda/tier esgota. Motivo real da 260821c: a label `trade-off-real` da #5891 entrou às 22:37Z, DEPOIS da única varredura da Fase 0 e fora de janela de re-checagem — a issue ficou invisível pra sessão inteira até a convergência da Fase 2, já com `deixado-para-o-300` gravado. Label/classificação muda mid-sessão por ação de OUTRA sessão (continuo, overnight paralelo, editor no Studio); esperar o fim do tier é dar à mudança uma janela de horas de cegueira evitável por um comando de ~1s. O resultado alimenta a mesma reclassificação da re-checagem entre ondas (issue que virou develop/bloqueada/agendada sai da fila corrente; novata entra pela onda certa).
...
1. **Sync:** `git checkout master && git pull`; capturar `base_sha = git rev-parse HEAD`; **registrar esta sessão no registro compartilhado (#5156):** `npx tsx scripts/lib/session-registry.ts register --kind develop` (`--session-id` sempre auto-injetado por `.claude/hooks/inject-session-id.mjs` — nunca passado manualmente, a skill não sabe o próprio `session_id`) — é o que permite ao overnight (item 3) checar claim de issue antes de dispatchar, ao merge lock (item 4) serializar merges cross-sessão, e ao guard de cleanup de worktree (item 9) detectar esta sessão como ativa. **Esta skill não precisa chamar `session-registry.ts heartbeat` só pra manter `lastHeartbeat` fresco** (#6327): `.claude/hooks/session-beacon.mjs` (`PreToolUse`) já faz isso sozinho a cada chamada de ferramenta desta sessão, enriquecendo ESTE MESMO registro em vez de criar um paralelo — **mas desde o #8495 esta skill CHAMA `heartbeat` explicitamente, com outro propósito**: publicar `active_worktrees` na abertura do fan-out de cada onda e no fechamento pós-cleanup (ver Fase 1 e Fase 2 abaixo, e item 6 na seção "Paralelismo entre sessões"). As duas chamadas não se substituem — o beacon cobre o campo `lastHeartbeat`, o call site explícito cobre `active_worktrees`, que o beacon não toca. Não reduzir/desligar esse hook sem primeiro ler a docstring de `heartbeat` em `scripts/lib/session-registry.ts` — a claim desta sessão depende dele pra não ficar presa em `claimed_issues` de outra sessão por até 90min (`SOFT_STALE_MS`) além do necessário. Ver seção "Paralelismo entre sessões (#5156)" no fim deste arquivo. **capturar `started_at` = timestamp ISO 8601 REAL desta sessão AGORA** (`date -u +%Y-%m-%dT%H:%M:%SZ`, ou equivalente) — gravado em `started_at` no `plan.json` do passo 9 abaixo. **Nunca gravar a string `{AAMMDD}` nesse campo** (#3841 — mesmo bug de raiz do overnight: `{AAMMDD}` nomeia só o diretório da sessão, não é uma data parseável; `started_at` precisa ser um ISO real pro painel `/rodada` do Studio conseguir ordenar TODAS as rodadas — overnight e develop — numa sequência cronológica única). `gh auth status`. **Log de startup do modelo/effort do coordenador (#3454, espelha #2993 do overnight):** logar no run-log o modelo/effort **CONFIGURADO** pelo frontmatter desta skill (`sonnet` / `high`) — não o auto-relatado —, pra tornar o pin verificável via `/diaria-log {AAMMDD}`:
   ```bash
...
3. **Varredura completa dos issues abertos** (#4319 — antes era só uma varredura de confirmação por label de bloqueio; agora precisa do backlog inteiro pra montar as 4 ondas): `npx tsx scripts/fetch-open-issues-for-triage.ts` (#7018 — mesmo CLI do overnight, sobre `scripts/lib/issue-triage-fetch.ts`; substitui o `gh issue list --json ...` escrito à mão — o campo `body` vem garantido ou o comando falha alto, nunca degrada `agendada`→`overnight` em silêncio; a resposta já traz `execTrack` via `classifyExecTrackFromListItem`, reusável no passo 6a abaixo em vez de recalcular); `updatedAt` continua no retorno por outros consumidores do JSON — a comparação de decisão do passo 4 NÃO usa esse campo, compara `decided_at`/`recorded_at` entre si — #6961; reconciliar contra o herdado do passo 2 — **fresh-scan vence o plan.json herdado em divergência**. Mesmo filtro `--bugs` se aplica ao resultado fresco (flag nativa do script). Mesmo filtro `--priority` se aplica ao resultado fresco (flag nativa do script).
...
8. Aplicar `--issues`/`--only`/`--bugs`/`--priority`/`--track` (filtra ANTES da onda ser fixada em `plan.json` — Goal avaliado contra o que sobrar). `--track {overnight|develop|agendada|bloqueada|fora-de-rodada}` (#5708) restringe a sessão ao que `classifyExecTrack` (não a classificação em prosa desta skill) atribuiu a cada issue — é o jeito do editor pedir literalmente "faça o que o painel chama de Develop": `--track develop` ataca só as issues que o painel já rotula Develop, mesmo que a onda/categoria em prosa desta Fase 0 as tivesse posto em outro lugar. Compõe com `--issues`/`--only`/`--bugs`/`--priority` (interseção, não união). Sem a flag, comportamento atual sem mudança — o filtro por track de prosa (ondas 1a-3) continua sendo o padrão. **Gravar o escopo no `plan.json` no mesmo passo (#6635/#6636):** `--bugs` → `bugs_only: true`; `--priority` → `priority_filter: [...]`; `--issues` → `issues_filter: [...]` (top-level, números crus) — é o que `filterIssuesByRoundScope` (`scripts/lib/state-changed-tracker.ts`) lê pra excluir issue fora do escopo explícito do ruído "novo não-triangulado" no gate de convergência da Fase 2; sem gravar, uma sessão `--issues`/`--bugs`/`--priority` vê o resto do backlog aberto inteiro como "novo" nesse gate (achado ao vivo #6635, sessão 260829).
...
**Escopo do Gate 1 residual, estreitado no #5695 — cat. A/B não re-perguntam mid-sessão.** Cat. A (credencial) e cat. B (conta de terceiro) são bloqueio de INSUMO: o editor já teve a chance de colar tudo de uma vez na Fase 0.5 (item 3/4). Uma issue cat. A/B que chega na Fase 1 sem `unblock_status: desbloqueada-validada` — seja porque não front-loadou, seja porque apareceu numa re-checagem entre ondas — **não gera um novo `AskUserQuestion`**: o coordenador checa deterministicamente se o insumo já existe (`.env` tem a var referenciada / a conta já foi confirmada em turno anterior) e, se sim, valida (#573) e segue pra implementação sem perguntar; se não, aplica o default direto — `pulada` motivo `nao-destravavel-na-sessao`, comentário durável explicando o que falta — com um banner (`Issue #N (cat. {A|B}) sem insumo disponível — pulada por default, sem novo pedido mid-sessão. Cole o insumo a qualquer momento pra reabrir.`), não uma pergunta bloqueante. O editor continua podendo colar um token/confirmar uma conta espontaneamente a qualquer momento — isso nunca foi "a skill perguntando", é o editor interrompendo por iniciativa própria (mesmo tratamento de toda intervenção mid-sessão nesta skill), e reabre a issue normalmente. **Exceção (#7946): issue cat. A/B pedida nominalmente via `--issues N,M` nesta invocação não cai no default silencioso** — o pedido explícito por aquela issue específica é sinal forte o bastante pra justificar UM `AskUserQuestion` pontual pelo insumo faltante (o motivo do #5695 pra não perguntar — evitar repetir pergunta de issue que ninguém priorizou — não se aplica a uma issue que o editor acabou de nomear). **O que sobra no Gate 1 residual é cat. C (sempre), cat. E quando o workaround tem custo real (critério 3 do #5321), e cat. A/B pedida via `--issues` sem insumo (exceção acima)** — essas continuam gerando `AskUserQuestion` (batchada por categoria quando aplicável). Cat. D nunca passa por Gate 1 — vai direto pro Gate B (seção Gates).

O Gate 1 remanescente (cat. C/E-com-custo, e cat. A/B pedida via `--issues` sem insumo — exceção #7946 acima) é serial mas **agrupa desbloqueios da mesma categoria numa única `AskUserQuestion`** (até 4 perguntas). **Com `goal_policy ∈ {exhaust_all, blocked_only}`:** trabalho acontece **um tier por vez** (`goal.current_tier`, começa em `1a`) — só issues do tier corrente entram na fila de implementação; um tier só é aberto pra trabalho depois que o anterior está esgotado (todo status terminal). **Com `goal_policy = table_only`:** não existe conceito de tier — `current_tier` não é gravado (schema, seção `plan.json`), todas as issues da tabela impressa na Fase 0 formam uma fila única, sem partição nem avanço por onda; o passo 6 abaixo é pulado inteiro nesse caso. **No início de cada iteração, reler `plan.json`.** **Antes do passo 1, rodar o "Filtro do servidor 300" (#5751, ver seção própria no topo deste arquivo)** pra cada issue do tier corrente sem status terminal — só segue pro passo 1 quem bate ≥1 dos 4 critérios (cat. A-E, `windows`, pedida nesta conversa, já em progresso nesta sessão); o resto vira `pulada` motivo `deixado-para-o-300` sem consumir Gate 1.

1. **Diagnosticar cada issue bloqueada do tier corrente sem `unblock_status: desbloqueada-validada`.** Cat. A/B: checar deterministicamente se o insumo já existe (`.env`/confirmação de turno anterior) — presente → passo 3 direto; ausente **e a issue não foi pedida via `--issues` nesta invocação** → default `pulada` motivo `nao-destravavel-na-sessao` com banner (ver acima), **sem** formular pedido de `AskUserQuestion`; ausente **e pedida via `--issues` (#7946)** → formular o pedido do insumo faltante pro passo 2, como cat. C/E. Cat. C/E-com-custo: formular o pedido de desbloqueio **exato e acionável** (ex: cat. C — "escolha A ou B pra {trade-off}"; cat. E — "workaround custa {N}, aplico?") pro passo 2. **Bloqueio SEM categoria A-E mapeada** (ex: veto da whitelist AAARRR, `matched: "label:aarrr-fora-da-whitelist"`) **pedido via `--issues` (#7946):** sem pergunta — registrar o override como comentário durável na issue + banner no terminal (ver nota após a tabela "Categorias de bloqueio") e tratar como elegível, direto pro passo 3; sem `--issues`, esse bloqueio não tem protocolo aqui e a issue não deveria ter chegado ao tier corrente (ver nota após a tabela "Categorias de bloqueio"). Issues sem sinal de bloqueio (tiers 1a/2/3 elegíveis) pulam direto pro passo 3.
2. **Gate 1 — desbloqueio** (`AskUserQuestion`, formato por categoria, pras issues cat. C/E-com-custo do tier corrente e cat. A/B pedida via `--issues` sem insumo (exceção #7946) — cat. A/B **sem** esse pedido explícito já resolveram no passo 1 sem pergunta) — toda opção inclui sempre "não consigo decidir agora (documentar e pular)".
...
4. **Compor a onda:** quando há ≥1 issue pronta (validada, ou elegível sem bloqueio) no tier corrente, rodar a análise de cluster de conflito (mapear arquivos por issue via grep no corpo + símbolos citados — puro lookup, sem escrita de código) → issues que colidem em arquivo **fundem numa unidade só** (ver "Paralelização segura", #4319). **Checar claim de outra sessão antes do fan-out (#5156 item 3):** pra CADA issue da onda, `npx tsx scripts/lib/session-registry.ts is-claimed --issue {N}` — `claimed: true` (outra sessão overnight/develop ativa já está trabalhando essa issue) remove a unidade da onda (status `pulada`, motivo `claimed-por-outra-sessao`, sem comentário — corrida evitada, não bloqueio); registro vazio nunca bloqueia. **`is-claimed`/`claim-issue` sempre como comando standalone, nunca em `&&`/`;`/pipe/heredoc** (`context/overnight-dispatch-rules.md` item 18, #5751) — comando encadeado quebra a injeção automática de `--session-id`, e o erro resultante ("--session-id ausente") é falha real, nunca ruído a ignorar. **Consultar `conflicts` ANTES de abrir worktree (#6168 Parte C, ponto 1):** `npx tsx scripts/lib/session-registry.ts conflicts --paths {arquivos da unidade, separados por vírgula} --branch {branch atual}` (comando **standalone**, `--session-id` auto-injetado — encadear quebra a injeção, item 18 de `context/overnight-dispatch-rules.md`). `exit 1` = sobreposição real com peer VIVO; `exit 0` = livre. Diferente do `is-claimed`, que responde por ISSUE, isto responde por ARQUIVO — é o buraco 2 da #6168 ("a coordenação é por issue, o conflito é por arquivo"): duas sessões em issues diferentes, sem colisão de claim nenhuma, editando o mesmo arquivo. **`conflicts` é CONSULTA, nunca aquisição** — não cria arquivo nem reserva nada; o que fazer com a resposta é decisão desta skill. Sobreposição com peer alcançável → avisar o peer (`SendMessage`, ver "Parte D" abaixo) antes de dispatchar; peer inalcançável → registrar a sobreposição em `plan.json` e seguir, nunca travar por silêncio.

**Preflight de duplicidade ANTES do dispatch (#7020) — obrigatório pra toda issue da onda, logo após os dois checks acima.** Antes de reivindicar/abrir worktree, rodar `npx tsx scripts/check-issue-duplicate-preflight.ts --issue N --updated-at {updatedAt da issue}` (CLI sobre `scripts/lib/issue-duplicate-preflight.ts`, que faz `git log origin/master --grep "#N"` — nunca `--all`; item 21 de `context/overnight-dispatch-rules.md`). Custa um `git log`, não um subagente. Três vereditos: **`not-in-master`** (exit 0) — dispatch normal, issue segue na onda sem mudança. **`closes-should-be-closed`** (exit 1) — algum commit em `origin/master` já usa `Closes #N` e a issue segue aberta: remover da onda de implementação, tratar como CLOSEOUT barato (confirmar que o commit cobre o pedido, comentar e fechar — nunca abrir worktree pra reimplementar o que já está em master), gravar `pulada` motivo `ja-resolvida-antes-da-sessao` com `ja_resolvida_evidencia` citando o SHA/commit do preflight (obrigatório — `validate-develop-plan-motivo.ts`, #5723). **`refs-declared-residue`** (exit 1) — commit já cita `#N` sem `Closes` (resíduo declarado ou citação incidental): reler o corpo da issue contra o diff desse commit e, se ainda sobrar escopo real, dispatchar só o resíduo — nunca o pedido original inteiro. `resolvedAfterLastUpdate: true` no resultado reforça "resolvida depois, ninguém reavaliou" — citar no comentário de closeout/redução de escopo. O preflight do subagente (item 14 do mesmo checklist) continua como rede de segurança, não substituído por este passo.

**Preflight de PR ABERTA, logo em seguida (#7788) — cobre o estado que o preflight acima não vê.** O passo anterior só enxerga trabalho já MERGEADO em `origin/master`; uma PR aberta cobrindo a mesma issue (típico de `continuo`, que abre PR a cada ~60min e drena a cada ~120min) é invisível pra ele — achado ao vivo: dois dispatches da rodada overnight 260909 custaram ~420k tokens só pra descobrir, já dentro da sessão, que uma PR `continuo/fix-*` aberta cobria o escopo. Rodar `npx tsx scripts/check-issue-open-pr.ts --issue N` (CLI sobre `scripts/lib/issue-open-pr-check.ts`, 1 chamada `gh pr list`). Três vereditos: **`no-open-pr`** (exit 0) — dispatch normal. **`open-pr-covers-scope`** (exit 1) — aplicar o checklist de 3 perguntas do item 16 de `context/overnight-dispatch-rules.md` (autor conhecido? CI verde/rodando? atualizada nas últimas ~24-48h?) antes de decidir esperar; falhando qualquer uma, tratar como se a PR não existisse e dispatchar normalmente. **`cannot-verify`** (exit 2) — `gh` indisponível/erro: **nunca** tratar como "sem PR aberta"; reter/retry, não degradar pra dispatch silencioso.

**Checar o teto de concorrência somando TODAS as sessões ativas (#5156 item 6):** `npx tsx scripts/lib/session-registry.ts list-active` — o teto de 6 worktrees abaixo é POR SESSÃO; se outras sessões ativas nesta máquina já somam `active_worktrees` significativos, considerar abrir menos que 6 nesta onda (heurística, não trava — throughput, não correção, mesmo espírito do "se a máquina engasgar, baixar manualmente é seguro" já documentado abaixo). Depois de passar pelos dois checks: → **Gate de Onda** (banner + segue direto — **pulado por default desde #5695**, `wave_policy = auto`; bloqueante só com `wave_policy = per_wave`) → reivindicar cada issue aprovada (`npx tsx scripts/lib/session-registry.ts claim-issue --kind develop --issue {N}`, uma chamada por issue) — **o `claim-issue` vale pra toda unidade que o coordenador decide trabalhar, via subagente OU DIRETAMENTE (sem fan-out — comum em unidades pequenas, Chrome-assisted, ou investigativas que não justificam worktree isolado): reivindicar antes de começar a mexer na issue em qualquer um dos dois caminhos, não só no de fan-out (#5407)** → **fan-out de até 6 implementadores concorrentes** (um worktree por unidade; branch `develop/fix-NNNN` — ou `develop/fix-NNNN-MMMM` pra unidade fundida — solo, ou `develop/blast-NNNN` p/ cat. D, sempre solo e nunca fundida). Cada PR passa pelo **review de fleet pré-merge** (#4383 — ver "Gates", "REVIEW DE FLEET PRÉ-MERGE"; o coordenador dispatcha os 5 agentes explicitamente, nunca o subagente implementador) e só então pelo **Gate 2 determinístico** (#2210/#2222), mergeando independentemente; unidade fundida fecha **todas** as issues do cluster no mesmo merge (`Closes` por issue, verificar `gh issue view --json state` pós-merge pra cada uma — **com o fechamento confirmado, soltar a claim na mesma respiração**: `npx tsx scripts/lib/session-registry.ts unclaim-issue --kind develop --issue {N}` por issue do cluster, #6317 — é o call site que fecha o caso concreto da issue: sem isso a claim só cai quando a SESSÃO INTEIRA termina, segurando issue já mergeada por horas). **Imediatamente após dispatchar o fan-out desta onda, publicar a contagem de worktrees (#8495 — call site de ABERTURA, obrigatório, espelha #6299 do overnight):** rodar `npx tsx scripts/lib/session-registry.ts heartbeat --kind develop --active-worktrees {N}`, onde `{N}` = worktrees desta sessão vivos agora (soma de todos os fan-outs concorrentes ainda não limpos, não só os desta onda). **Standalone, nunca em `&&`/pipe/heredoc nem dentro de um bloco de código multi-linha** (`context/overnight-dispatch-rules.md` item 18) — encadear quebra a injeção do `--session-id` e a publicação vira no-op silencioso, mesmo critério de `test/skill-chained-session-command-guard-6232.test.ts`. **Fila grande (>8 issues prontas no tier, #2754):** a análise de cluster de conflito pode ser delegada a um subagente `general-purpose` com `model: haiku` explícito (não `sonnet` — aqui é puro grep/leitura, sem julgamento de implementação; latência menor sem perda de qualidade) em vez do coordenador rodar serialmente — só essa etapa de mapeamento, nunca a implementação em si (que continua sempre `sonnet`, #2019).
...
- **GATE 2 — IMPLEMENTAÇÃO/MERGE** (determinístico, sem humano, por PR): gate de 2 condições do overnight (#2210/#2222 — condição 1: `npx tsx scripts/check-pr-checks-gate.ts --pr {N}` sai `exit 0` (ver `scripts/lib/pr-checks-gate.ts`; substitui `gh pr checks --json bucket`, que não roda no `gh` do `300`, #6225) **E** condição 2: threads não-resolvidas excluindo FORBIDDEN == 0 via `gh api graphql --jq`), em chamada separada do `gh pr merge`, + verify #573. **Merge lock cross-sessão antes do `gh pr merge` (#5156 item 4):** `npx tsx scripts/lib/session-registry.ts merge-lock-acquire --pr {N}` (`--session-id` auto-injetado). **`--pr` não é opcional na prática** — achado ao vivo #7169/#7223 (rodada no 300, PR #7217): sem ele, `gh pr merge` foi bloqueado 4× seguidas pelo guard #5716 mesmo com a sessão registrada como coordenadora e o lock em mãos; `--pr {N}` explícito destravou de primeira (reproduzido também por uma sessão interativa peer no PR #7218). `denied` → outra sessão (overnight ou develop concorrente) está no meio do próprio merge+pull (TTL 2 min) — esperar ~15s e tentar de novo, poucas tentativas, escalando pra halt banner se persistir. **Nota (#6182): entre máquinas via OneDrive, o lock é advisory — `O_CREAT|O_EXCL` vê inodes diferentes.** **Mitigação adicional pós-#7169:** rodar `git fetch origin master` e conferir se o head mudou desde a última leitura antes de confiar no lock/grant cross-máquina — é sinal que vem do GitHub, não do OneDrive, e atravessa máquina de verdade mesmo com `data/sessions/` sincronizando degradado (achado ao vivo #7169: `onedrive.service` morto por 1h13 sem alarme deixou o registro cego entre `Neo` e `300`). Liberar com `merge-lock-release --pr {N}` (mesmo `--pr {N}` do acquire, #7169/#7223) logo após o `git pull` que segue o merge — a janela protegida é só `gh pr merge` até esse `git pull`, nunca mais que isso. **O guard do #5716 agora CONSULTA o lock (#6296):** uma coordenadora que não o detém enquanto OUTRA coordenadora ativa existe é bloqueada — pegar o lock deixou de depender de a skill lembrar. Lock ausente com esta sendo a única coordenadora, ou estado do lock ilegível, nunca bloqueiam (nunca transformar falha transitória de I/O do OneDrive em bloqueio de merge legítimo).

  **Conceder janela a uma sessão que não é coordenadora (#6296).** Quando uma sessão interativa (ou qualquer sessão fora do registro) precisar mergear enquanto esta rodada está ativa, o guard do #5716 a bloqueia — corretamente, porque ela não é coordenadora e não vira uma por relabel. O caminho legítimo é esta sessão CONCEDER a janela, depois de conferir colisão por arquivo contra os PRs abertos da rodada: `npx tsx scripts/lib/session-registry.ts grant-merge --kind develop --granted-to {sessionId dela} --pr {N}` (`--session-id` auto-injetado). TTL de 10 min, **uso único**, e **ninguém concede a si mesmo** — é isso que preserva a propriedade que o #5716 protege em vez de contorná-la. **Passe `--pr` sempre:** a concessão é ESCOPADA àquele PR (#6303) — sem o flag ela vale pra qualquer merge daquela sessão durante o TTL, o que anularia a conferência de colisão que justifica concedê-la. **No `SendMessage` que avisa a beneficiária, inclua explicitamente que o `gh pr merge` dela precisa citar o número do PR** (`gh pr merge {N} ...`, nunca deixar o `gh` inferir pela branch corrente) — concessão escopada só cobre o PR exato dela (#6322): sem o número explícito, `extractGhPrMergeTargetPr` não consegue casar a chamada com a concessão e o guard bloqueia mesmo com janela válida em mãos. A sessão beneficiada pode confirmar com `check-merge-grant`, mas **não precisa consumir à mão**: o hook `PostToolUse` `.claude/hooks/consume-merge-grant-on-merge.mjs` consome a janela sozinho após o `gh pr merge`. Isso é deliberado — a beneficiária é tipicamente uma sessão interativa que nunca leu este arquivo (ela só recebeu um `SendMessage`), então depender dela lembrar seria o mesmo "depende de skill lembrar" que o #6168 existe pra eliminar. **Nunca** destravar alguém mandando rodar `register --kind develop`: isso fabrica identidade de coordenadora e fura o guard (era o único caminho que funcionava antes do #6296, e é o que ele existe pra tornar desnecessário).

  **Trem de merge (#6300, decisão do editor 26/08/2026, default ATIVO nas 3 skills — overnight/develop/contínuo).** Quando ≥2 PRs do lote/onda corrente já satisfazem o Gate 2 (condição 1 verde, condição 2 zero threads) AO MESMO TEMPO e não colidem em arquivo entre si (mesma análise de cluster de conflito de "Paralelização segura" acima), preferir `npx tsx scripts/run-merge-train.ts --session-id {sessionId desta sessão} --kind develop --prs {N1,N2,...}` a mergear 1-a-1 — colapsa K runs de CI num só (economia medida na issue: até 1,89 runs por PR hoje, contra 1 run por lote com o trem) e mergeia sob o MESMO `merge-lock-acquire`/`release` já documentado acima (a chamada é interna ao script, não precisa repetir manualmente). `--session-id` é **obrigatório e explícito** aqui — diferente de uma chamada Bash direta desta sessão, o script chama `session-registry.ts` como subprocesso PRÓPRIO, fora do alcance do hook `inject-session-id.mjs`. Lote de 1 PR só (nenhum outro Gate-2-verde no momento) → não vale a pena, mergear direto como sempre. **Degrada sozinho, nunca trava**: CI do lote vermelho ou timeout (30 min, mesma convenção de sempre) bissecta automaticamente até o piso — um PR ruim isolado no meio de um lote bom nunca prende os demais. Commit final é **1 squash por lote com `Closes` de todas as issues** (mesma convenção da fusão de cluster colidente, #4319 — revert é tudo-ou-nada pro lote). Ver `scripts/lib/merge-train.ts`/`merge-train-live.ts` pro mecanismo completo; `scripts/plan-merge-train.ts` mostra o plano sem executar, útil pra inspecionar antes.
...
**Gate de cobertura do `target_set` (#5718), logo em seguida:** rodar `npx tsx scripts/check-develop-target-set-coverage.ts --plan data/develop/{AAMMDD}/plan.json`. `exit 1` → pelo menos uma issue de `goal.target_set` chegou até aqui sem entrada em `issues[]` (ou com entrada sem `status`) — foi triada, fazia parte do escopo da sessão, e nunca recebeu nenhuma tentativa de dispatch. Pra cada número listado: adicionar `{ number, status: "nao-tentada" }` a `issues[]` — backfill mecânico, sem julgamento (não é reclassificar a issue, é só registrar que esta sessão não a tocou), regravar o `plan.json`, rodar de novo até `exit 0`. **Se alguma dessas issues chegou a ser reivindicada** (`claim-issue` rodou numa onda anterior a este backfill — caso raro, mas possível quando o dispatch falhou logo após a claim), soltar com `unclaim-issue --kind develop --issue {N}` antes de seguir (#6317) — `status: "nao-tentada"` e uma claim ativa da própria sessão contam a mesma história contraditória que `deixado-para-o-300` conta (ver Filtro do servidor 300 acima). Complementa os dois gates acima: aquele cobre issue nova fora do plano, este cobre motivo inventado numa issue tentada, este aqui cobre issue conhecida que nunca foi tentada — sem ele, o número ficava solto em `goal.remaining` e a Seção de Relatório abaixo não tinha como distinguir "tentei e travei" de "nunca tentei".

**Gate de cobertura do `exec_track_painel` (#5907 item a), logo em seguida:** rodar `npx tsx scripts/check-develop-exec-track-coverage.ts --plan data/develop/{AAMMDD}/plan.json`. `exit 1` → dois modos de falha: (1) entrada(s) de `issues[]` sem `exec_track_painel` — o passo 6a da Fase 0 não rodou pra ela (o modo real da 260821c: o `classifyExecTrack` estourou timeout, foi pro background e a sessão seguiu sem ele; nada pegava a ausência); (2) valor gravado fora do enum de 5 tracks (`overnight|develop|agendada|bloqueada|fora-de-rodada`) — typo vira classificação que nenhum consumidor (`findDeixadoPara300Buraco` #5914, painel de Triagem #5462) reconhece. Pra cada número listado: rodar `classifyExecTrack` e gravar o campo — backfill mecânico, sem julgamento (o módulo é puro), regravar o `plan.json`, rodar de novo até `exit 0`. Complementa o gate do gap (b) (#5914): aquele recusa `deixado-para-o-300` em track develop/bloqueada QUANDO o campo existe; este garante que o campo exista pra TODA entrada — sem os dois, o buraco do 300 fica invisível exatamente nas sessões que mais precisam dele.

**Gate de drift de label de decisão (#5892), logo em seguida:** rodar `npx tsx scripts/check-decision-label-drift-gate.ts --plan data/develop/{AAMMDD}/plan.json`. Mesmo script do overnight — detecta drift entre a prosa da sessão e as labels estruturais atuais (racional completo, incluindo as duas fontes de prosa do #5955 e o procedimento de resolução: `context/overnight-dispatch-rules.md` item 25). `exit 0` (nenhum drift) → seguir pro relatório. `exit 1` (drift detectado) → aplicar as labels estruturais faltantes reportadas (procedimento no item 25 citado acima) antes de continuar — o relatório NÃO compila enquanto houver achados não resolvidos.

**Lint de prosa de dependência sem marcador (#7137 item 4), logo em seguida — AUDITORIA, nunca bloqueia (diferente do gate acima).** Rodar `npx tsx scripts/check-dependency-prose-lint.ts`. Mesmo script do overnight — reporta issues abertas que citam prosa de dependência ("pré-requisito", "depende do #N", "depois que #N", "só depois") sem o marcador `<!-- depends-on: #N -->` (item 24, `reconcile-issue-dependencies.ts`). Correção exige julgamento (dependência genuína ou falso positivo?), por isso **sempre `exit 0`**: os achados entram como linha no relatório, nunca travam a compilação. Racional completo: `scripts/lib/dependency-prose-lint.ts`.
...
**Gate de SAÍDA do track Develop (#6271), logo em seguida:** rodar `npx tsx scripts/check-develop-label-cleared.ts --plan data/develop/{AAMMDD}/plan.json`.

**O problema que ele fecha: Develop é um sink.** **Medido em `origin/master`, ANTES deste gate existir** (a própria adição abaixo muda a contagem, então re-rodar o `grep` hoje dá outro número — isto é registro de evidência congelado, não invariante): das 6 ocorrências de `route-issue` neste arquivo, **uma única era de SAÍDA** — remover `trade-off-real` ao fechar uma cat. C — e ela cobre 1 das 3 labels que disparam `develop`. Para `windows` e `develop-track` não havia instrução nenhuma (`grep -c "remove-label windows"` → **0**). Issue entra no track e não sai, exceto por ação humana ou por sorte; a fila do painel cresce monotonicamente com trabalho já feito, e o editor perde justamente a distinção que a Triagem (#5462 / épica #5969) existe pra dar.

**Que a prosa sozinha não basta, este arquivo já provou** — a instrução de cat. C traz o raciocínio escrito desde o #5462 e ainda assim foi esquecida (#5415), o que motivou o guard irmão do #5821. Medição do #6271, na MESMA sessão e com o mesmo coordenador, a ~1h de distância: na #6098 ele rodou `route-issue` ao notar que o código já estava mergeado; na #6181 **não** rodou, mesmo tendo acabado de consumir integralmente a razão do `windows`. Resultados opostos, mesma disciplina — ou seja, não havia disciplina, havia sorte.

`exit 1` → para cada issue listada, responder **"a razão que a trouxe pro Develop ainda existe?"**:
- **não existe mais** → `npx tsx scripts/route-issue.ts --issue N --track {overnight|bloqueada} --reason "..."` (`overnight` quando o resto é trabalho comum; `bloqueada` quando passou a depender de outra issue — foi o caso da #6114 → #6269);
- **ainda existe** (ex: parte 1 mergeada, parte 2 segue exigindo Chrome logado) → gravar o porquê em `develop_track_justificado` na entrada da issue no `plan.json`. Mesma disciplina de `unblock_evidence`/`ja_resolvida_evidencia`: o motivo é aceito, mas precisa estar escrito — **o que o gate recusa é o silêncio, não a permanência.**

Só entram na checagem issues que ESTA sessão **terminou de trabalhar** (`mergeada`, `entregue-fora-de-codigo`, ou `pulada` motivo `ja-resolvida-antes-da-sessao`). Issue `nao-tentada` ou `deixado-para-o-300` **nunca é acusada** — ela continua sendo fila legítima do Develop, e é exatamente o caso das 2 issues corretas da medição (#6048, #467). O gate **não decide o track de destino**: ele detecta o resíduo e para; qual é o track verdadeiro é julgamento da sessão, registrado via `route-issue` (mesmo princípio do #5373 — julgamento feito uma vez por quem tem contexto, gravado, nunca re-derivado por heurística).

Sem `gh`/rede, **dois casos distintos** (#738): se TODAS as buscas falharem, o gate não rodou e a saída diz isso explicitamente — não é veredito de "sem resíduo"; se apenas ALGUMAS falharem, o gate roda sobre cobertura PARCIAL, e a linha de veredito carrega a ressalva com a contagem das não consultadas. Nos dois casos `exit 0` (fail-soft), mas nunca um "ok" que esconde o que não foi checado. `plan.json` ausente/ilegível é erro duro (`exit 2`), como o gate acima.
...
**Razão adição:remoção (#7113) — mesma proeminência das issues destravadas/mergeadas acima.** Rodar `npx tsx scripts/measure-round-diff-stats.ts --base {commit-base da sessão} --session-kind develop` (persiste em `data/run-log.jsonl`), depois `npx tsx scripts/round-diff-stats-report.ts --check-alarm` — colar a tabela `janela | rodadas | adições | remoções | razão | líquido/dia` (7d/30d/90d) impressa no digest. `--check-alarm` reconcilia sozinho uma issue de alarme (via `scripts/lib/alarm-issues.ts`) se a razão de 7d cruzar o limiar — nada a decidir aqui além de colar o output.
...
```
File-based (grava em `data/reports/index.jsonl`) — nunca depende do `npm run studio` estar no ar; o comando imprime a URL em stdout, capturar pro resumo do terminal. **O comando não manda mais e-mail (#7960, item 4 da #7957)** — `--no-email` continua aceita por compat mas virou no-op; até o #7960, omitir a flag mandava e-mail real pro editor a cada invocação (#5521 — foi assim que uma auditoria de 73 relatórios históricos disparou ~50 e-mails de uma vez em 17/08/2026), hoje o relatório develop é "Studio /relatorios, sem e-mail" na tabela de severidade do editor. **Não criar mais draft via `create_draft` aqui.** **Preceda a tabela de unidades do relatório com o marcador literal `<!-- unidades-mergeadas -->` (#5521)** — com ele, o número de unidades no título (`"N unidades"`/`"N PRs"`) tem que bater com as linhas da tabela e o de issues (`"N issues"`) com as issues distintas que elas cobrem; divergência sai `exit 1` sem registrar nada, e o comando imprime o sufixo correto. Sem o marcador o comando avisa em stderr que a conferência não rodou. Único ponto não fail-soft do comando, de propósito (o título vira assunto de e-mail, que não tem desfazer); rationale completo no passo equivalente de `.claude/skills/diaria-overnight/SKILL.md`. Canal primário = terminal, incluindo a linha `Relatório: {URL do Studio}`; fail-soft #738 — falha da gravação em disco (rara) nunca trava a sessão, só avisa que o relatório ficou local; o e-mail é best-effort separado, não coberto por essa garantia.

**Check de SAÍDA (#6168 Parte C, ponto 3), antes de encerrar o registro:** rodar `npx tsx scripts/lib/session-registry.ts conflicts --branch {branch atual}` uma última vez e conferir o próprio registro. Os dois pontos de entrada (antes de worktree, antes de commit) cobrem o começo; as duas evidências mais caras da #6168 são de SAÍDA e nenhum deles pegava:

- **registro não encerrado** — uma sessão `/diaria-develop` ficou listada como ativa com `claimed_issues: [6128, 6181]` DEPOIS de terminada (#6128 já CLOSED, PR #6183 já MERGED). Sem intervenção manual, as duas issues ficariam invisíveis pra qualquer outra sessão. `end` só é chamado nesta Fase 2, então qualquer parada fora dela deixa o registro órfão;
- **trabalho sem commit** — um tick do contínuo terminou deixando 4 arquivos modificados e 1 untracked em `master`, no checkout compartilhado, e reportou "concluído". Qualquer sessão que rodasse `git add -A` no intervalo publicaria trabalho de outra frente na PR errada.

Então, antes do `end`: `git status --porcelain` limpo? (sujo → commitar, stashar ou mover pra fora do repo — **nunca** encerrar deixando trabalho não commitado em `master` num checkout compartilhado); toda issue com PR mergeada já saiu de `claimed_issues`? `conflicts` acusando `branch-drift` (a branch registrada não é mais a do checkout) → outra sessão trocou o checkout embaixo, checar antes de encerrar. Registrar o resultado no relatório.

**Limpeza de worktrees mergeados (#4335, follow-up do #4326):** por último, depois do relatório salvo/registrado, **encerrar o registro desta sessão primeiro** (`npx tsx scripts/lib/session-registry.ts end --kind develop`, `--session-id` auto-injetado — #5156, senão esta própria sessão aparece como "outra sessão ativa" pro guard abaixo) e só então rodar `npx tsx scripts/cleanup-merged-worktrees.ts` — varre `.claude/worktrees/` (worktrees concorrentes desta sessão + sobras de rodadas anteriores), confirma via `gh pr list --head {branch} --state merged` quais branches já foram mergeadas e remove (`git worktree remove --force`) só essas. **FAIL-SOFT**: o script já nunca lança nem sai não-zero por falha de `gh`/permissão; mesmo assim, tratar qualquer stderr como warning — nunca travar o encerramento da sessão por causa desta limpeza. **Guard de sessão compartilhada embutido no próprio script (#5156 item 9):** ele consulta `session-registry.ts` e PULA a varredura inteira, sozinho, se detectar qualquer outra sessão ainda ativa nesta máquina (ex: um overnight que começou depois desta sessão develop) — não insistir com `--confirm-shared` sem certeza de que não há colisão. **Em seguida, republicar a contagem (#8495 — call site de FECHAMENTO, obrigatório, par do call site de abertura na Fase 1):** rodar `npx tsx scripts/lib/session-registry.ts heartbeat --kind develop --active-worktrees {N}` com `{N}` já refletindo os worktrees removidos por `cleanup-merged-worktrees.ts` acima — nesta skill o cleanup roda em LOTE no fim da sessão (não por PR individual como no overnight), então este é o ponto real onde o número de worktrees vivos desta sessão muda de verdade. **Standalone**, mesma disciplina de injeção do call site de abertura.
...
**Fronteira revertida (#7493, decisão do editor 05/09/2026).** O `/diaria-overnight` voltou a levar as ambíguas de trade-off-real ao **seu próprio briefing** (Fase 0 passo 5), em vez de bounceá-las pra cá — a label `trade-off-real` classifica `overnight`, não `develop`. As DUAS famílias de ambiguidade (trivial-mas-não-documentada e trade-off real de produto/editorial) são hoje perguntas do mesmo briefing.

Motivo: sob a fronteira do #2640, o overnight passou 20 rodadas seguidas (260828→260905) sem classificar **nenhuma** issue como `precisa-resposta` — e como a aprovação de agrupamento e o opt-in de loop estendido pegam carona na mesma `AskUserQuestion`, o briefing inteiro parou de perguntar. A ambiguidade virava roteamento em vez de pergunta, e o roteamento custa uma sessão supervisionada inteira contra 1 pergunta numa janela em que o editor já está presente.

**O que sobra pro develop nas ambíguas:** a **cat. C continua existindo** como categoria de trabalho (ver a tabela A–E) — o que mudou é a via de ENTRADA. Uma issue não chega mais aqui *por carregar a label* `trade-off-real`; ela chega quando o editor a traz nesta sessão, quando a decisão exige também algo que só o develop faz (`windows`, `develop-track` — essas duas vencem `trade-off-real` na precedência de `classifyExecTrack`), ou quando o `/diaria-desbloqueia` a encaminha. Quando uma cat. C for de fato decidida aqui, o fechamento é o mesmo de sempre: comentário durável + remoção da label + `check-trade-off-label-cleared.ts`.
...
- Stall passivo é inaceitável (#738): toda espera de CI usa `gh pr checks --watch` em background; timeout de CI = 30 min → tratar como CI vermelho. **Preferir `scripts/lib/wait-pr-checks.sh {N}` a um laço escrito à mão** (#6921: laços sem teto mecânico ficaram órfãos no `300` por até 15h) — o helper já embute os 30min desta regra no próprio processo.
...
6. **Teto de concorrência por sessão** — MECANIZADO (#8495, fecha o #5161 fleet review item 7): read-path (`session-registry.ts list-active` soma `active_worktrees` de outras sessões antes de decidir quantos worktrees abrir nesta onda, ver "Paralelização segura" e Fase 1 passo 4 acima) e write-path (`heartbeat --active-worktrees N`) mecanizados dos dois lados — esta skill agora tem os dois call sites obrigatórios (ao dispatchar o fan-out de cada onda, Fase 1; e após o cleanup em lote de `cleanup-merged-worktrees.ts`, Fase 2), espelhando os dois que o `/diaria-overnight` já chama desde o #6299 (ao dispatchar a onda e após o cleanup de cada worktree). **Diferença de forma, não de garantia:** o overnight decrementa por worktree individual conforme cada um é limpo; esta skill decrementa uma vez por LOTE no fim da sessão, porque `cleanup-merged-worktrees.ts` só roda ali (ver Fase 2 acima) — a soma cross-sessão fica correta nos dois casos, só a granularidade do decremento intermediário difere. Antes do #8495, só o overnight publicava e o pior caso (desde o #8486, teto 3→6) chegava a 12 worktrees na máquina sem nenhuma das duas sessões enxergar a outra — fechado.
...
**Resumo do que ainda é risco aceito, não mecanizado (ou só parcialmente):** item 5 (corrida rara no checkout compartilhado, mitigada pelo merge lock do item 4) e a metade não-wired do item 11 (duas sessões develop na mesma máquina ainda podem se misturar na statusLine até `plan.session_id` ser populado). Item 6 fechou no #8495 (write-path `heartbeat --active-worktrees` agora tem os dois call sites nesta skill, espelhando o overnight desde o #6299).
