...
description: Lista todos os erros intencionais declarados pelo editor nas edições de um mês (concurso "ache o erro, ganhe um número"). Lê `_internal/intentional-error.json` de cada edição em `data/editions/{YYMM}*/` (fallback pra `data/intentional-errors.jsonl` em edições pré-#3222) e agrega numa tabela markdown com categoria, localização, descrição e valor correto. Identifica edições sem declaração. Argumento — YYMM (ex — 2605 para maio 2026).
---
...
2. Output stdout — markdown formatado com:
   ...
   - Lista de edições sem declaração (com motivo: arquivo ausente, `_internal/intentional-error.json` ausente, campos faltando, etc.)
...
4. Se houver edições sem declaração, sugerir ao editor:
   > N edições sem `intentional_error` declarado. Forneça os campos via chat pra cada edição — grave `_internal/intentional-error.json` retroativamente pra fechar o histórico.
...
- **260506**: intentional_error_missing: _internal/intentional-error.json ausente ou inválido
