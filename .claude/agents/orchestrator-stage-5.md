...
**So dispatchar se o canal foi autorizado em 5b** (canal manual fica com `status: pending_manual`) **e se `chrome_mcp` nao for `false`** (lido na Pre-condicao acima, #5414) — com `false`, pular direto pra gravar `status: "skipped"` em vez de gastar o playbook pra falhar.
...
2. **Para cada item de `posts`**, chamar `mcp__claude_ai_Buffer__create_post` com `channelId` = `publishing.social.twitter.buffer_channel_id` de `platform.config.json`, `text` = o texto extraido, `mode: "customScheduled"`, `dueAt` = `posts[i].dueAt` (ja calculado no passo 1), `schedulingType: "automatic"`. **Quando `posts[i].imageUrl` nao e `null`** (#4264), incluir tambem `assets: [{ image: { url: posts[i].imageUrl, metadata: { altText: posts[i].altText } } }]` — mesma arte que Facebook/Instagram/LinkedIn levam para esse destaque. Quando `imageUrl` e `null` (ver `skipped_image` do passo 1), omitir `assets` e postar so texto, sem abortar o destaque. **Nao usar `addToQueue`** (decisao do editor 260727, #4103 — reverte 260726): a fila propria da Buffer nao tem relacao com os horarios editoriais dos demais canais e dessincroniza (caso real 260727: Facebook/LinkedIn/Instagram/Threads saindo 10:00/12:30/17:30 enquanto o X da fila saiu 08:55/09:10/10:57 — dois destaques quase colados e fora de ordem de divulgacao). **Nao usar `shareNow`** tambem — o agendamento explicito via `dueAt` e o unico dono do cronograma.
