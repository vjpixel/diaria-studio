...
1. `Bash("npx tsx scripts/prep-twitter-posts.ts --edition-dir {EDITION_DIR}/")` — le `publishing.social.twitter` de `platform.config.json` (gate `enabled`) e a secao `# Curto` de `03-social.md` (texto unico compartilhado com Threads, #3992) — **sem fallback**: destaque ausente na secao ou texto >280 chars entra em `skipped`, nunca improvisa texto nem trunca em silencio. Retorna JSON `{enabled, published_path, posts: [{destaque, text, dueAt, imageUrl, altText}], skipped, skipped_image}` — `dueAt` (#4103) ja vem calculado pelo MESMO `compute-social-schedule.ts` usado por `publish-facebook.ts`/`publish-linkedin.ts`/`publish-instagram.ts`/`publish-threads.ts` (fallback_schedule/timezone unico de `platform.config.json`), nunca recalculado a parte. `imageUrl`/`altText` (#4264) vem de `06-public-images.json` (MESMO criterio de `publish-instagram.ts`: card `d{N}_4x5` com titulo > 1:1 `d{N}` sempre presente) — quando o cache nao existe ou nao tem a entry, `imageUrl` vem `null` e o motivo entra em `skipped_image` (post continua saindo, so texto — X sem imagem e pior que X nenhum, nunca vira um `skipped` de verdade). Se `enabled: false` ou `posts` vazio, **pular os passos 2-3** (nada a publicar).
2. **Para cada item de `posts`**, chamar `mcp__claude_ai_Buffer__create_post` com `channelId` = `publishing.social.twitter.buffer_channel_id` de `platform.config.json`, `text` = o texto extraido, `mode: "customScheduled"`, `dueAt` = `posts[i].dueAt` (ja calculado no passo 1), `schedulingType: "automatic"`. **Quando `posts[i].imageUrl` nao e `null`** (#4264), incluir tambem `assets: [{ image: { url: posts[i].imageUrl, metadata: { altText: posts[i].altText } } }]` — mesma arte que Facebook/Instagram/LinkedIn levam para esse destaque. Quando `imageUrl` e `null` (ver `skipped_image` do passo 1), omitir `assets` e postar so texto, sem abortar o destaque. **Nao usar `addToQueue`** (decisao do editor #4103 — reverte tentativa anterior; caso real que motivou a reversão: `docs/orchestrator-stage-narrative-history.md#stage-5-buffer-queue-caso-real`). **Nao usar `shareNow`** tambem — o agendamento explicito via `dueAt` e o unico dono do cronograma.
3. **Apos cada `create_post`**, gravar o resultado com `Bash("npx tsx scripts/append-twitter-published.ts --published-path {published_path} --destaque {d} --status {published|scheduled|draft|failed} --url {url} --buffer-post-id {id} --scheduled-at {dueAt}")` (usar `--reason` em vez de `--url`/`--buffer-post-id`/`--scheduled-at` se `create_post` retornar erro) — `{published_path}` vem do output do passo 1, `{dueAt}` e o mesmo valor de `posts[i].dueAt` passado no passo 2.

Erro em `create_post` para um destaque nao aborta os demais — best-effort, mesmo padrao dos outros canais social (#2486).
