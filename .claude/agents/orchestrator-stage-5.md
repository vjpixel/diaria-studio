...
5. `Bash("npx tsx scripts/publish-twitter.ts --edition-dir {EDITION_DIR}/")` — **SEM `--schedule`** (#3994 v1: X free tier nao tem agendamento nativo; post sai imediato no instante deste dispatch, horario aceitavel pro canal). Le a secao `# Curto` de `03-social.md` (texto unico compartilhado com Threads, #3992) — **sem fallback**: se a secao nao existir, pula com log, nunca improvisa texto. Assina via OAuth 1.0a User Context (`scripts/lib/twitter-oauth1.ts`). **Bloqueado ate o editor criar a conta X da Diar.ia + app no Developer Portal** (#3994, status confirmado 260724) — sem `TWITTER_API_KEY`/`_SECRET`/`_ACCESS_TOKEN`/`_ACCESS_TOKEN_SECRET` no env, o script **encerra com exit 0** (skip gracioso), nao bloqueia os outros canais.
