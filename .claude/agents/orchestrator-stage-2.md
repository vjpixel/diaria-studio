...
- **Render ERRO INTENCIONAL obrigatório (#1073):** após Clarice (e antes do gate humano), rodar:
  ...
  npx tsx scripts/render-erro-intencional.ts \
    ...
    --md {EDITION_DIR}/02-reviewed.md
  ```
  Substitui o placeholder do writer pelo reveal do erro anterior (`Na última edição, …`) + preserva ou insere placeholder pra `Nessa edição, …` (autor preenche manualmente). Também garante que `_internal/intentional-error.json` existe (escreve placeholder `{PREENCHER}` se ausente — #3222, campos estruturados description/location/category/correct_value/wrong_value/reveal — `wrong_value` desde #7243, sibling de `02-reviewed.md`, **nunca** sincroniza com o Drive). **Falha = abortar Stage 2** (não silenciar). Justificativa: sem o script, edição vai com `{placeholder, script render-erro-intencional.ts substitui pós-Clarice}` literal no MD; quando colado manualmente no Beehiiv (#1083), aparece como texto bruto no email — contamina UX e mata o concurso "Ache o erro".

  **Coletar os campos do editor (#3222).** Diferente de antes (editor editava o frontmatter YAML direto no Google Doc via Drive), o JSON não passa pelo Drive. **Este é o único ponto do inventário do #5321 que continua perguntando (critério 2 — conteúdo editorial autoral, não dá pra decidir por conta própria o que está "errado")** — mas em vez de abrir a pergunta no vazio, monte uma **proposta completa e pronta pra aceite em 1 clique** aplicando o filtro de segurança do #3808 abaixo (descrição / localização / categoria / valor correto / **valor errado efetivamente plantado no texto (`wrong_value`, #7243)** / frase de reveal já preenchidos), e apresente no gate como "aceito esta proposta / quero outra / vou escrever a minha" — nunca como um formulário de 6 campos em branco. Gravar a resposta direto em `{EDITION_DIR}/_internal/intentional-error.json`, substituindo os placeholders `{PREENCHER}`. **`wrong_value` importa tanto quanto `correct_value`** — é o que permite ao Stage 4 (`intentional-error-present-in-final`, `scripts/lib/invariant-checks/stage-4.ts`) detectar mecanicamente se o item com o erro sumiu do texto numa poda de RADAR do gate (incidente real: edição 260902, reveal da 260903 ia publicar afirmação falsa — #7243); sem preenchê-lo, esse guard não bloqueia, só avisa. Isso é o único lugar onde o editor "edita" esse dado agora — nunca via Drive. **Escopo (#7214): a regra "erro intencional só humano" dos agentes de escrita (`writer`/`writer-destaque`/`social-writer`/`social-curto`) restringe SÓ ELES — não você. Ela existe porque o writer escreve sem visão do conjunto da edição; você, orquestrador, tem essa visão, e é exatamente por isso que propor a candidata é o seu trabalho, não uma violação da mesma regra.**

  **Modo headless / auto-approve (`--no-gates`, #8592).** Nesse modo ninguém responde a coleta acima no Stage 2. Rode o render com `--auto-approve` (acrescente a flag ao comando acima): o bloco recebe a narrativa padrão "Nessa edição também tem um erro plantado, ache e responda pra concorrer." em vez de `{PREENCHER_NARRATIVA_DO_ERRO}`, e `check-stage2-invariants.ts --headless` emite um WARN (não bloqueante) `intentional_error_pending_headless` enquanto os 5 campos de `_internal/intentional-error.json` seguem `{PREENCHER}`. **Não invente nem plante um erro automaticamente** — a escolha é editorial e precisa passar pelo filtro #3808 abaixo; em headless o candidato é PROPOSTO no gate 4 ("proponha e plante um erro em 1 clique").
