---
name: eia-composer
description: Stage 4 — Busca a Foto do Dia da Wikimedia (POTD), gera versão AI similar via Gemini, sorteia A/B (#192), escreve `01-eia.md` (com frontmatter `eia_answer`) + `01-eia-A.jpg` + `01-eia-B.jpg`.
model: haiku
...
```bash
curl -sL "{url}" -o "{out_dir}/01-eia-real-raw.jpg"
```
...
```bash
# O script eia-compose.ts sorteia qual slot (A ou B) recebe a foto real.
# Nomes finais: 01-eia-A.jpg e 01-eia-B.jpg (nunca real/ia legacy).
npx tsx scripts/crop-resize.ts \
  {out_dir}/01-eia-real-raw.jpg \
  {out_dir}/01-eia-{realSide}.jpg \
  --width 800 --height 450
```

Se o crop falhar, retornar erro. Após sucesso, remover o arquivo raw: `Bash("rm {out_dir}/01-eia-real-raw.jpg")`.

### 2b. Registrar uso no log

Após download bem-sucedido, anexar a imagem escolhida em `data/eia-used.json` via script dedicado (args passados pelo shell — imune a aspas/entidades nos metadados da Wikimedia):
...
node scripts/gemini-image.js \
  {out_dir}/_internal/01-eia-sd-prompt.json \
  {out_dir}/01-eia-{aiSide}.jpg \
  diaria_eia_
```

Se falhar com exit code != 0, retornar erro — não prosseguir sem a imagem IA.

### 4. Escrever `{out_dir}/01-eia.md`
...
```json
{
  ...
  "ai_image_file": "01-eia-ia.jpg",
  "real_image_file": "01-eia-real.jpg",
  "ai_side": null,
  "wikimedia": {
...
```json
{
  "out_md": "data/editions/260418/01-eia.md",
  "out_real": "data/editions/260418/01-eia-A.jpg",
  "out_ia": "data/editions/260418/01-eia-B.jpg",
  "out_meta": "data/editions/260418/_internal/01-eia-meta.json",
  "image_title": "...",
  "image_credit": "...",
  "image_date_used": "2026-04-15",
  "rejections": [
...
> **Nota:** os valores de `out_real` e `out_ia` acima usam A e B como exemplo; o slot real/ia é definido pelo sorteio em `eia-compose.ts` e pode ser A ou B em qualquer edição. Consultar `_internal/01-eia-meta.json` (campo `ai_side`) para saber qual slot contém a imagem IA.

`rejections` é opcional mas deve ser incluído quando houve fallback (dias pulados). `out_meta` aponta pro `_internal/01-eia-meta.json` (passo 5) — orchestrator preserva no resume e `publish-newsletter` lê pra preencher `ai_side` ao inserir as imagens.
