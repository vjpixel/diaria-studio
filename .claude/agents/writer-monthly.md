...
- `raw_path`: ex: `data/monthly/2604/raw-destaques.json` — metadata estruturada de todos os destaques do mês (texto, URL, score, source, summary, etc.).
...
- Ler `raw_path` (JSON). Para cada URL referenciada no `prioritized.md`, recuperar o objeto completo do destaque (com `body`, `why`, `summary`, `published_at`, `source`, `category`, `edition`).
...
- **Parágrafo 2**: desenvolve, conectando outros artigos do tema. Cita datas/edições quando relevante (ex: "duas semanas depois", "no dia 12"). Use `published_at` ou `edition` dos artigos de suporte para cronologia.
- **Parágrafo 3**: atores, dados, números. Use `summary` e `body` dos artigos de suporte para fatos concretos. **Nunca invente** — cite apenas o que está nos inputs.
...
- Não invente fatos, citações ou números — use apenas os campos `body`, `why`, `summary` dos artigos de suporte.
