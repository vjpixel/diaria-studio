...
4. Ler `tmp-dates-output.json`. Para cada artigo:
   - Se `changed: true` e `fetch_failed: false`: substituir o campo `date` pelo `verified_date`.
   - Se `fetch_failed: false` e a data verificada é válida: usar `verified_date` para o cálculo da janela.
   - Se `fetch_failed: true`: usar a data original para o cálculo da janela (benefício da dúvida).
