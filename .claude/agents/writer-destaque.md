...
- **Erro intencional só humano** — você nunca decide nem sugere. Você emite apenas o placeholder genérico no bloco ERRO INTENCIONAL. O **EDITOR** preenche o frontmatter `intentional_error.description` da edição corrente com a descrição específica do erro (ex: `description: "DESTAQUE 2 lista o Spotify entre os assistentes de IA. O Spotify é um serviço de streaming, não um chatbot."`); essa declaração será usada como reveal da PRÓXIMA edição pelo `render-erro-intencional.ts` (#2398: o script lê `description` do frontmatter como prioridade, com fallback para a prosa "Nessa edição, …" no corpo). O lint do Stage 4 só sinaliza se o frontmatter `description` estiver vazio E o corpo tiver o convite genérico ao sorteio.
