...
Se Gmail MCP indisponível: pular silenciosamente (logar `info "0-replies skipped: Gmail MCP unavailable"`). Nunca bloqueia a edição. Se `pre_gate !== true` (headless `--no-gates` sem `--session-supervised`, ou skill isolada sem `pre_gate`): pular silenciosamente (logar `info "0-replies skipped: headless --no-gates"`). **`--no-gates` sozinho não é mais sinônimo de "sem editor" (#6719)** — desde que `/diaria-edicao` spawna o Stage 1 num processo próprio (#5744), `--no-gates` é passado a TODO spawn independente de o editor estar supervisionando a sessão-mãe; `pre_gate` só chega `true` aqui quando `/diaria-1-pesquisa` também recebeu `--session-supervised` (ver `.claude/skills/diaria-1-pesquisa/SKILL.md`).
