...
- Se `_internal/05-published.json` existe **e** `status === "skipped"` (Chrome MCP estava indisponível) → **re-probar Chrome MCP** (`mcp__claude-in-chrome__tabs_context_mcp`). Se probe suceder: **gravar `--set chrome_mcp=true`** (#5414 — sem isso o `false` antigo fica no disco e o Stage 5, que agora lê de lá, pula a publicação de novo apesar do Chrome estar funcionando), deletar o arquivo marcador e tratar como se Etapa 5 não tivesse rodado. Se probe falhar: pular para auto-reporter e gravar `--set chrome_mcp=false`.
...
- **Pre-flight token Cloudflare/wrangler (#2286).** O `CLOUDFLARE_API_TOKEN` expirado só estoura em `maintain-valid-editions` (§0d.bis) — depois de gastar tokens em dedup e CTR. Checar ANTES, análogo ao check-google-token:
  ...
  ```
  Exit 0 = ativo OU erro de rede transitório (não bloqueia pipeline — soft note no stderr). Exit 1 = ausente/inválido/não-ativo → stderr traz banner com ação (`wrangler login` ou renovar no `.env`). (Exit 2 removido em #2306 — transitório agora sai 0.) Se exit 1, **default (#5321): alertar o editor com o banner e seguir sem bloquear** (impacto registrado: `maintain-valid-editions` e KV do É IA? vão falhar no §0d.bis — o editor renova o token quando quiser). Gravar `--set cloudflare_token_ok=false` se exit 1 (e `=true` se exit 0) — §0d.bis usa pra decidir se tenta ou salta com halt.
- **Pre-flight Clarice REST (#1329).** Pinga `https://cortex.clarice.ai/api-correction` antes do Stage 2 saber se o fallback REST está saudável. Não bloqueia — só armazena `clarice_rest` (`true`/`false`) no preflight-state:
  ```bash
  npx tsx scripts/clarice-healthcheck.ts
  ```
  Output JSON: `{ ok, latency_ms?, error? }`. Exit 0 = saudável (`--set clarice_rest=true`); exit 2 = degradado (`--set clarice_rest=false`, logar warn com `error` e seguir). Stage 2 §3b consulta `--get clarice_rest` antes de tentar o fallback quando o MCP falha. Sem essa flag, Stage 2 ainda tenta o fallback — só perde a chance de pre-warn o editor.
> **#5414 — todo probe desta secao e PERSISTIDO, nao guardado "em sessao".** Logo apos cada um, gravar:
> ```bash
> npx tsx scripts/lib/preflight-state.ts --edition-dir {EDITION_DIR}/ --set {chave}={true|false}
> ```
> Chaves: `chrome_mcp`, `gmail_mcp`, `beehiiv_mcp`, `clarice_rest`, `cloudflare_token_ok`. E isso que permite Stage 2/5/6 rodarem em **contexto proprio** (sessao limpa) sem perder o que o preflight descobriu — a heranca de contexto entre stages e ~49% do custo de uma edicao. Ler depois com `--get {chave}`, que imprime `true`/`false`/`unknown`. **`unknown` (nao probado) nunca deve ser lido como `false`** — a acao correta e re-probar; tratar como `false` faria a edicao pular publicacao em silencio.

- **Pre-flight Claude in Chrome MCP (#143).** Tentar `mcp__claude-in-chrome__tabs_context_mcp`. Setar `chrome_mcp=true` se sucesso, `chrome_mcp=false` se erro (via `preflight-state.ts --set`, ver nota acima).
...
**Antes de tentar, ler o probe do Cloudflare (#5414)** — não depende de lembrar do §0c:
```bash
npx tsx scripts/lib/preflight-state.ts --edition-dir {EDITION_DIR}/ --get cloudflare_token_ok
```
`false` → o token já falhou no preflight; este passo vai falhar também. Registrar o warn e seguir (o editor renova quando quiser). `true` ou `unknown` → tentar normalmente.
