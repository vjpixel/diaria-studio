...
3. Filtrar quais são respostas de assinante (determinístico):
   ...
   ```
   (assunto `Re:` + remetente humano — exclui automáticos `no-reply`/`beehiiv`/`mailer-daemon` e os próprios endereços do editor.) **#4095: cada reply do array `replies[]` também vem marcada com `trivial: boolean`** (`stripQuotedAndSignature` + `isTrivialReply` — corpo sem substância: saudação/agradecimento isolado, vazio, ou só emoji/pontuação, depois de limpar citação da newsletter + assinatura). A marcação **não remove** a reply do array — ela só decide, no passo 4.6 abaixo, se vale a pena criar rascunho. Os passos 4.1–4.5 (crédito do concurso "ache o erro") rodam para **TODAS** as replies, trivial ou não — pular o rascunho nunca pode pular a checagem do erro intencional.
