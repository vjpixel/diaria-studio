...
`erro-intencional-placeholder` (#2078, #3489): validado como invariante GATE-BLOCKING já no Stage 2 (`check-invariants.ts --stage 2`, antes do gate de Stage 2) — narrativa `{PREENCHER_NARRATIVA_DO_ERRO}` no bloco ERRO INTENCIONAL de `02-reviewed.md`, ou campos `{PREENCHER...}` de `_internal/intentional-error.json` (description/location/category/correct_value/reveal — ver `orchestrator-stage-2.md` §2a "Coletar os campos do editor"), ainda não preenchidos. Não deveria sobreviver até o Stage 4 — **mas se sobreviver** (Stage 2 pulado/bypassado, retomada de checkpoint pré-preenchimento), é **GATE-BLOCKING aqui também**: não montar o gate (§4d) com placeholder pendente. **Antes de PROPOR um candidato de erro sozinho pra fechar essa lacuna** (caso o Stage 2 tenha pulado essa etapa), aplicar por padrão, ANTES de apresentar qualquer proposta ao editor, o mesmo **Filtro de segurança ao PROPOR candidatos (#3808)** já documentado em `orchestrator-stage-2.md` §"Filtro de segurança ao PROPOR candidatos" — as 3 diretrizes de `context/editorial-rules.md` §10 ("Concurso 'ache o erro'"): (1) nunca alterar um fato central de um DESTAQUE (funding, specs, dado de negócio — o que o leitor levaria como "notícia real"); (2) preferir erro cômico/leve (trocadilho, erro ortográfico bobo, nome trocado de forma óbvia) a inflação de magnitude sobre um fato real; (3) preferir plantar em menção lateral/secundária, nunca na frase que carrega a informação principal do destaque. Montar a proposta completa (os 5 campos, já filtrada pelas 3 diretrizes) e apresentar como "aceito esta proposta / quero outra / vou escrever a minha" — nunca uma pergunta em aberto sem o filtro aplicado primeiro (caso real que motivou esta nota: `docs/orchestrator-stage-narrative-history.md#stage-4-erro-intencional-safety-filter-gap`, #5566). Ação após aceite: gravar em `_internal/intentional-error.json`, rodar `render-erro-intencional.ts` (§2a) se ainda não rodou, e re-checar `erro-intencional-placeholder` antes de prosseguir pro gate.
