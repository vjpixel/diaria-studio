...
**4c.8 — Rampa Gmail: exibir o que o Stage 0 já mediu (#7021, resíduo do #7041):** ler `{EDITION_DIR}/_internal/kit-gmail-ramp-proposal.json` (gravado pelo passo 0o de `scripts/stage-0-run.ts`, se existir — fail-soft por natureza, não roda em toda edição). Ausente → `{ramp_gmail_line}` = `↷ não avaliada nesta edição (sem canal Kit anterior ou medição falhou — fail-soft, ver run-log)`. Presente → 1 linha só: `waveSkipped` → `gate segurou — {waveReason} (edição {sourceEdition})`; senão → `onda de {waveSize} endereço(s) proposta ({safeToTagCount} seguro(s) já, {needsBeehiivDeactivationCount} precisa(m) desativação manual na Beehiiv) — aplicar: {applyCommand} — atencao: o `--push` faz DUAS coisas, marca a tag no Kit E desativa o endereco na Beehiiv`. **Puramente informativo, nunca bloqueia o gate — este passo NUNCA roda `--push`** (aplicar a onda é ação manual do editor, guard de publicação do item 1 de `context/overnight-dispatch-rules.md`; sem gate novo, é informação ao lado do gate de conteúdo já existente).
...
- `{ramp_gmail_line}` (#7021) = 1 linha lida de `kit-gmail-ramp-proposal.json` (§4c.8) — nunca bloqueia o gate; não confundir com aprovação, é só exibição do que o Stage 0 já mediu.
