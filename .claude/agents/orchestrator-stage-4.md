...
```
(#5416) Este comando substitui as invocações separadas que existiam aqui antes (`post_pixel-matches-d1`, `no-xml-artifacts`, `no-antithesis-reveal`, `no-trailing-editorial-hook` — e, desde o #6862, também `no-markdown-emphasis` — 1 processo Node cada) por uma única chamada agregadora — mesmas funções, mesmo veredito por check. Output (stdout): JSON `{ stage: 4, passed: boolean, checks: [{ id, source_issue, severity, ok, result }, ...] }`. `passed` já reflete "nenhum check falhou" — todos os 5 `id` deste stage são `gate-blocking` (não há warn-only em §4c.2b). Pra cada `checks[]` com `ok:false`, tratar como ❌ (mesma ação descrita no parágrafo do check correspondente abaixo, indexado pelo `id`). Debug de 1 check isolado (`--check X --md {EDITION_DIR}/03-social.md`) continua funcionando exatamente como antes (modo aditivo, não removido).
...
`no-markdown-emphasis` (#6862) — **GATE-BLOCKING:** achado ao vivo (31/08/2026) — `**`/`__`/`*`/`_` de ênfase saindo LITERAL no texto publicado (nenhuma plataforma social renderiza markdown; editor confirmou em Instagram, LinkedIn e Facebook, corrigiu os 2 primeiros à mão). ❌ mostrar no resumo com ação: "remova a marcação de ênfase das linhas listadas antes de aprovar — nunca editar 03-social.md achando que resolve na fonte: o carrossel (`gen-carousel-cards.ts`) lê o MESMO arquivo e MANTÉM o negrito de propósito (decisão do editor, 31/08/2026); a correção real vive no ponto de publicação (`stripMarkdownEmphasis`, `scripts/lib/strip-markdown-emphasis.ts`), chamado pelos 5 publishers (`publish-facebook.ts`, `publish-instagram.ts`, `publish-threads.ts`, `publish-linkedin.ts`, `publish-eia-social.ts`) — se este check falhar, o bug é em algum desses arquivos não chamar a função, não no `03-social.md`". Gate só pode ser aprovado (`sim`) após lint verde.
