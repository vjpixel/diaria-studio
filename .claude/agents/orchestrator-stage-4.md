...
**Gate de aprovação:** antes de aceitar `sim` em §4d, checar `npx tsx scripts/stage4-cascade-status.ts --edition-dir {EDITION_DIR}/ --status` — exit 1 (`pending: true`) significa que a cascata do último ajuste ainda não terminou; **esperar** (síncrono) até resolver antes de aceitar a aprovação — o editor não deve aprovar uma edição com imagem/social visivelmente desatualizados no preview. Erro do script (arquivo ilegível etc.) é fail-soft — tratar como "nada pendente" e seguir, nunca travar o gate por uma falha de infraestrutura de UX.
