...
- **#2608 — Estimar queries Path B após dispatch.** Após despachar todos os agents, emitir: `npx tsx -e "import {recordBraveCreditEstimate} from './scripts/lib/brave-credits.ts'; recordBraveCreditEstimate({edition:'{AAMMDD}',source:'stage1-agents',count:{N}*2+{M}+{J}})"` onde N=source-researchers, M=discovery-searchers, J=launch_candidates discovery; **multiplicador 2=média por source-researcher (fragilidade: se o prompt do source-researcher mudar para 3 queries/fonte, a estimativa subnotifica ~33% — `delta_untracked` no relatório revelaria o gap via header X-RateLimit-Remaining)**. Entries com `estimated:true` são distinguidas de calls reais no relatório.
