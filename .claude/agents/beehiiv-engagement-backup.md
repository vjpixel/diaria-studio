...
3. **Aplicar via stdin pipe, uma página por vez**:
   ...
   # identidade de clique (passo 2, list_post_click_subscribers) — SEMPRE --kind click-subscribers:
   echo '<JSON com {"engagement": clickPage}>' | npx tsx scripts/apply-mcp-subscriber-engagement.ts \
     --post-id X --kind click-subscribers --append
   ```
