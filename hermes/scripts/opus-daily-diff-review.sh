...
# estiver acontecendo". O transcript continua inteiro em $OUT_FILE (e no log
# do cron, via stderr); o que decide a mensagem é a linha de resumo.
#
# Problema aqui = findings > 0 (há o que olhar) ou issues_falharam > 0 (o
# review achou algo e NÃO conseguiu registrar — o pior caso, porque some).
# Resumo ilegível/ausente também avisa: não dá pra afirmar "está tudo bem" a
# partir de um resumo que não deu pra ler. Dia limpo (findings=0) é silêncio.
