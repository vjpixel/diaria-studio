#!/usr/bin/env bash
# Telegram recebe somente o resumo final; detalhes ficam no log.
exec 3>&1
exec 1>&2
...
exec 1>&3
exec 3>&-
echo "fim — revisadas=$REVIEWED mergeadas=$MERGED escaladas=$ESCALATED rejeitadas=$REJECTED falhas=$((FAILED+INFRA_ERRORS))$LOCK_NOTE"
