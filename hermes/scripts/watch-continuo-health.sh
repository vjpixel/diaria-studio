...
# das ~13 checagens imprimia sua linha "ok" no stdout, então uma varredura
# perfeitamente saudável virava uma mensagem longa todo dia. Pedido do editor:
# "só receber mensagem se algum problema estiver acontecendo".
#
# `note()` é pra tudo que é diagnóstico de rotina (inclusive os "ok"): vai pro
# LOG em disco, nunca pro Telegram. stderr fica reservado pro que é anômalo —
# quando FAILS>0 o script sai 1 e o Hermes entrega o stderr como alerta, então
# poluí-lo com as linhas "ok" tornaria esse alerta ilegível justo no dia em
# que ele importa. stdout só recebe o resumo final, e só quando há problema.
...
# de "algo aconteceu e precisa de você" — aí, e só aí, vai uma mensagem curta
# (título da issue, não o diagnóstico inteiro: o detalhe já está na issue).
...
# do próprio job de watch vira o alarme de quem vigia o vigilante. O Hermes
# entrega o stderr junto nesse caso; por isso as linhas de rotina foram pro
# $WATCH_LOG e não pra stderr — senão esse alerta nasceria ilegível.
[ "$FAILS" -eq 0 ] || exit 1
exit 0
