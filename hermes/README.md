...
O job do cron carrega `model`, `provider` e `reasoning_effort` **nos
próprios campos** de `~/.hermes/cron/jobs.json` — e esses vencem o
`model.default` de `~/.hermes/config.yaml`. Medido ao vivo no bench do
#7602 (08/09/2026): trocar só o `model.default` mudou o smoke por CLI, mas
o `cron run 5d791ef6fc2c` continuou disparando o modelo antigo.
...
Pra trocar o modelo do contínuo, editar o job em `jobs.json` (com backup e
o valor anterior registrado) e conferir depois pelo `billing_provider` de
