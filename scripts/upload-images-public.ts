...
async function main(): Promise<void> {
  ...
  // #1275: --no-require-keys opt-out de validação. #1418: --force-reupload
  // ignora cache md5/target e força upload de novo.
  const skipExisting = !flags.has("no-cache");
  const forceReupload = flags.has("force-reupload");
  ...
  const requireKeys = !flags.has("no-require-keys");
  if (requireKeys) {
