...
async function main(): Promise<void> {
  ...
  if (isDryRun) {
    if (orphans.length > 0) {
      ...
      console.log(`${LOG_PREFIX} nenhum e-mail necessário (sem órfão pendente).`);
    }
  } else if (orphans.length > 0) {
    // #7960: `shouldAlarmKitDoiOrphans(state, orphans)` gateava por
    // `computeKitDoiOrphanFingerprint(orphans) !== state.lastAlarmedFingerprint`
    // (fingerprint AGREGADO) — `"dedupe-new-occurrences-only"` sobre TODOS os
    // outcomes reproduz a idempotência: só reenvia quando ao menos 1 órfão é
    // genuinamente NOVO (issue `created`/`reopened`).
    const result = await notifyEditorForOutcomes(
      allFindingOutcomes,
      "acao",
      () => buildKitDoiOrphanAlarmEmail(orphans, now, issueRefs),
    ...
    if (result.qualifying.length === 0) {
      console.log(`${LOG_PREFIX} política '${result.emailPolicy}': nenhum e-mail necessário (conjunto já alarmado antes).`);
