...
<body>
  ...
  <table>
    <tbody>
      ...
      <tr><td>Uso este mes${braveCredits.alert_basis === "brave_header" ? " (header Brave — autoritativo)" : ""}</td><td><strong>${braveCredits.effective_used}</strong> / ${braveCredits.free_tier_limit} queries (${braveCredits.percent_used}%) — ~$${braveCredits.cost_this_month_usd.toFixed(2)} de $${braveCredits.monthly_free_credit_usd.toFixed(2)} crédito grátis mensal${braveCredits.alert_basis === "brave_header" ? ` <span class="warn">(contador local conta só ${braveCredits.queries_this_month} — subnotifica o Path B)</span>` : braveCredits.queries_this_month_estimated > 0 ? ` <em>${braveCredits.queries_this_month_real} reais + ${braveCredits.queries_this_month_estimated} estimadas (Path B)</em>` : ""}</td></tr>
      ${braveCredits.projected_month_end !== null ? `<tr><td>Projecao fim do mes</td><td>~${braveCredits.projected_month_end} queries (~$${braveCredits.projected_cost_month_end_usd?.toFixed(2)})</td></tr>` : ""}
      ...
      ${braveCredits.monthly_quota_unmeasurable ? `<tr><td>Header Brave (X-RateLimit-Remaining)</td><td><em>sem sinal mensal — conta Postpaid, sem cota mensal a decrescer (#7943)</em></td></tr>` : ""}
      ...
      <tr><td>Status</td><td class="${braveCredits.alert_level === "critical" ? "err" : braveCredits.alert_level === "warn" ? "warn" : ""}">${braveCredits.alert_level === "critical" ? "&#9888; Custo acima do esperado (&gt;95% do crédito grátis mensal — plano Postpaid, não bloqueia, só fatura)" : braveCredits.alert_level === "warn" ? "&#9888; Atenção (&gt;80% do crédito grátis mensal)" : "&#10003; OK"}</td></tr>
    </tbody>
  </table>
  ` : ""}
