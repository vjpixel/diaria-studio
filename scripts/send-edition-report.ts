...
<body>
  ...
  <table>
    <tbody>
      ...
      <tr><td>Uso este mes${braveCredits.alert_basis === "brave_header" ? " (header Brave — autoritativo)" : ""}</td><td><strong>${braveCredits.effective_used}</strong> / ${braveCredits.free_tier_limit} (${braveCredits.percent_used}%)${braveCredits.alert_basis === "brave_header" ? ` <span class="warn">(contador local conta só ${braveCredits.queries_this_month} — subnotifica o Path B)</span>` : braveCredits.queries_this_month_estimated > 0 ? ` <em>${braveCredits.queries_this_month_real} reais + ${braveCredits.queries_this_month_estimated} estimadas (Path B)</em>` : ""}</td></tr>
