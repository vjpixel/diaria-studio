 ...
 * (`.gitignore:25`, sem prefixo de path — vale pra QUALQUER worker, não só o
 * `poll`). Ele apenas espelha, como declaração de tipo, o `[vars]` do
 * `wrangler.toml` — que é o arquivo de fato policiado por esta regra e já está
 * na allowlist por causa do CORS do embed. Sem este skip, todo worker com
 * `ALLOWED_ORIGINS` reprova o `--static` na máquina de quem rodou
 * `wrangler types`, e nunca no CI (clone fresco não tem o arquivo).
 *
 * O skip é condicionado ao HEADER que o wrangler escreve, não só ao nome: um
 * arquivo homônimo escrito à mão — ou commitado à força por cima do
 * `.gitignore` — continua sendo policiado normalmente. Sem essa condição o
 * guard ganharia um nome de arquivo permanentemente cego, o que é justamente o
 * tipo de buraco silencioso que o #4059 existe pra fechar.
 */
function isGeneratedWranglerTypes(path: string, lines: string[]): boolean {
  if (basename(path) !== "worker-configuration.d.ts") return false;
  return lines.slice(0, 5).some((l) => /Generated by Wrangler/.test(l));
}
...
export function checkNoBeehiivHostInPublicCta(baseDir?: string): Violation[] {
  ...
  for (const dir of targetDirs) {
    ...
    for (const path of walkTs(dir)) {
      ...
      if (isGeneratedWranglerTypes(path, lines)) continue;
      for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
