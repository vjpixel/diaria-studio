...
export function saveState(state: SubscribeDriftAlarmState, statePath: string = STATE_PATH): void {
  mkdirSync(dirname(statePath), { recursive: true });
  writeFileAtomic(statePath, JSON.stringify(state, null, 2) + "\n");
}

// ─── Estado (dedup/reconciliação de ISSUE por achado) ──────────────────────
// Arquivo separado de STATE_PATH de propósito — mesmo racional de
// hub-drift-check.ts/worker-drift-check.ts: idempotência do E-MAIL (acima) e
// tracking de ISSUE por achado são preocupações independentes.
...
async function main(): Promise<void> {
  ...
  saveState(advanceSubscribeDriftState(nextFingerprint, new Date()));
}
