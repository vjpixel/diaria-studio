...
export function toAlarmFinding(hubSlug: string, entries: readonly AgedStaleHubEdition[]): AlarmFinding {
  ...
  return {
    ...
    body: [
      ...
      "  npx tsx scripts/build-hub-page.ts --all",
      "",
      "Se o 2º comando abortar citando \"updatedDate ... é anterior à edição\"",
      "ou \"data ... é posterior a updatedDate\" — ISSO NÃO É UM BUG (#4911/",
      "#5124): é o gate esperado pedindo uma passada editorial (ler a(s)",
      "edição(ões) nova(s), decidir se abre seção/arco, bumpar UPDATED_DATE em",
      `scripts/lib/hubs/${hubSlug}.ts com uma nota) — precedente: PR #7258.`,
      "Rode `npx tsx scripts/check-hub-editorial-gate.ts` (#7101/#7103) pra",
      "distinguir esse gate esperado de um defeito real de conteúdo ANTES de",
      "tentar regen — não depende de `data/`, roda em qualquer checkout.",
      "",
