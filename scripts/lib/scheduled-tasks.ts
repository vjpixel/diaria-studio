...
export const SCHEDULED_TASKS: ScheduledTaskDefinition[] = [
  ...
  },
  {
    ...
    // Diária 11:05, na banda matinal dos demais drift-checks e pelo mesmo
    // motivo (#5113): o conserto — mergear o PR de publicação, rodar o deploy —
    // é ação manual do editor de manhã, então detectar de madrugada não
    // antecipa nada.
    //
    // 11:05, e não 10:05 como a 1ª versão desta PR tentou: a hora das 10 está
    // LOTADA de 5 em 5 (:00 a :55), e 10:05 já é do Diaria-Ads-Spend-Ingest-Alarm.
    // O comentário original afirmava que o slot estava livre — errado, e o
    // review da PR #7595 pegou. O teste de colisão abaixo passou a existir por
    // causa disso.
    //
    ...
    schedule: { kind: "daily", hour: 11, minute: 5 },
  ...
  // #7406 (05/09/2026) — "Diaria-Clarice-Envio-Engajados" e
  // "Diaria-Clarice-Envio-Engajados-Alarm" (20:15/21:15 BRT, #6945) foram
  // APOSENTADAS aqui — nunca deveriam ter existido como automação separada
  // (decisão do editor: "não faz mais sentido ter grupos diferentes
  ...
  },
  {
    ...
    description:
      ...
      "anterior por canal, o acumulado do teste 2608 (se em andamento) e leitores adquiridos por canal -- " +
      "elimina a ambiguidade entre 'sem gasto' e 'task falhou'. Era e-mail diario ate o #7960 (item 4 da " +
      "#7957), #7487",
    ...
    // alarme de DEFEITO na ingestao -- este digest SEMPRE sai, papel
    ...
    // em modo cloud, aborta graciosamente sem registrar relatorio (mesmo
    // padrao de ads-test-watch.ts) -- nunca tenta produzir um digest sobre
    // dado ausente.
    ...
    guard: {
      ...
      abortMessage:
        "spend.csv nao encontrado (data/aquisicao/spend.csv) -- provavel junction data/ nao montada ainda; " +
        "abortando por seguranca, sem produzir digest sobre dado ausente.",
    },
    // DECLARADA, NAO ARMADA nesta unidade (worktree isolado, mesma
    // disciplina do resto do registro) -- armar via
    // `scripts/setup-systemd-timers.ts` na checkout compartilhada (`300`)
