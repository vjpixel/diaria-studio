 ...
 * `Z >= 1` é exigido — chama-se com 0 selecionados gera linha esquisita.
 */
export function formatCoverageLine(args: {
  ...
  return (
    `Para esta edição, eu (o editor) enviei ${args.editorSubmissions} submissões e a ` +
    `Diar.ia encontrou outros ${args.diariaDiscovered} artigos. Selecionamos os ` +
    `${args.selected} mais relevantes para as pessoas que assinam a newsletter.`
  );
}
