/**
 * lib/launch-heuristics.ts (#2833)
 *
 * Motor de classificacao de artigos em categoria editorial (Category) e o
 * mapeamento pra secao da newsletter (Bucket) -- todas as heuristicas de
 * deteccao de lancamento/pesquisa/tutorial/deal/update/customer-story/etc,
 * mais a funcao categorize() que orquestra a ordem de avaliacao.
 *
 * Extraido de scripts/categorize.ts -- movimentacao pura, sem mudanca de
 * comportamento. categorize.ts re-exporta esses simbolos pra manter compat
 * com importadores existentes e mantem apenas I/O (categorizeArticles + CLI).
 */
...
const CUSTOMER_STORY_PATTERNS: RegExp[] = [
  ...
  /\b\w+\s+helps?\s+(?!developers\b)\w+(\s+\w+){0,3}\s+(move|grow|scale|work|earn|build|automate|deliver)\b/i,
...
const NON_PRODUCT_ANNOUNCEMENT_PATTERNS: RegExp[] = [
  ...
  // Alliance for AI Safety and Security". Corpus não tem nenhum caso onde
  // "alliance"/"coalition" nomeia um produto vendável — sempre iniciativa/
  // consórcio.
  /\b(alliance|coalition)\b/i,
];
