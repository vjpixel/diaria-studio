...
export function renderEncerramento(body: string): string {
  ...
  if (pills.length) {
    ...
    // #2139: centralizar via table align="center" (email-safe; div/flex ignorados).
    parts.push(`<table role="presentation" align="center" cellpadding="0" cellspacing="0"><tr><td style="text-align:center;">${pills.join("")}</td></tr></table>`);
  }
  if (last) {
    parts.push(
