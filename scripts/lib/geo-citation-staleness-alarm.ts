...
export function buildGeoCitationStalenessAlarmEmail(
  ...
  lines.push(
    ...
    "  - todo provider (ANTHROPIC_API_KEY/OPENAI_API_KEY/GEMINI_API_KEY/PERPLEXITY_API_KEY) perdeu a key.",
    "",
    "Verifique:",
...
//
// Achado ao vivo (issue #5316): a Anthropic ficou muda em `300` desde
...
export function buildMissingProviderAlarmEmail(
  ...
  const lines: string[] = [
    ...
    "OPENAI_API_KEY / GEMINI_API_KEY; PERPLEXITY_API_KEY é opcional e não dispara este alarme) sumiu do .env da máquina que roda a",
