...
async function main(): Promise<void> {
  ...
  for (const { item, guardrail } of evaluations) {
    // #7960 self-review (PR #8297): `markEvaluated` fecha a campanha PRA
    // SEMPRE (nunca reavaliada) — antes da migração, um `sendGmailMessage`
    // que lançasse abortava `main()` ANTES deste `markEvaluated`, garantindo
    // retry na próxima execução. `notifyEditorForOutcomes` nunca lança, então
    // é preciso decidir explicitamente: só marcar avaliada quando o e-mail
    // saiu (ou quando não havia breach pra alarmar) — senão um push falho
    // perderia o alarme dessa campanha PERMANENTEMENTE.
    let emailConfirmedOrNotNeeded = !guardrail.anyBreach;
    if (guardrail.anyBreach) {
      ...
      if (isDryRun) {
        ...
        // Mesma idempotência de antes — cada campanha só é AVALIADA 1 vez na
        // vida (`markEvaluated`, abaixo), então o outcome pra essa campanha é
        // sempre "created" nesta 1ª (e única) avaliação; `legacyResendIntent`
        // default ("resend-every-run") preserva o comportamento literal (1
        // e-mail por campanha com breach).
        ...
        if (result.qualifying.length === 0) {
          // Política decidiu que este outcome nunca e-mailia (ex: issue
          // falhou ao criar/reusar) — nada mais a fazer aqui, não é retry-able
          // por este caminho; segue marcando avaliada como antes.
          emailConfirmedOrNotNeeded = true;
          ...
          console.error(`[clarice-guardrail-alarm] falha ao enviar e-mail (campanha "${item.name}"): ${result.emailError} — campanha NÃO marcada avaliada, retry na próxima execução.`);
        }
      }
    }

    if (!isDryRun && emailConfirmedOrNotNeeded) nextState = markEvaluated(nextState, item.id);
  }
