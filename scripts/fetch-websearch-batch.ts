...
async function runQuery(
  ...
  if (shouldRecordBraveResponse(response)) {
    recordBraveCredit({
      ...
      // (#7943) persist the monthly LIMIT alongside remaining — needed to detect
      // the "0 = no monthly cap" sentinel (Postpaid accounts) that makes
      // quota_remaining permanently uninformative for reconciliation.
      ...(typeof response.quota_limit_monthly === "number"
        ? { quota_limit_monthly: response.quota_limit_monthly }
        : {}),
    });
  }
