...
import { subscribeViaConfiguredBackend, resolveSubscribeUtm, type SubscribeDeps } from "./subscribe"; // #6291: função única, ramifica por SUBSCRIBE_BACKEND internamente
...
export async function handleJogarGateSubscribe(
  ...
  // de `subscribeViaConfiguredBackend` em subscribe.ts.
  const result = await subscribeViaConfiguredBackend(env, { name, email }, fetchImpl, utm);
  if (!result.ok) {
