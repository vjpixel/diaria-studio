...
import {
  ...
  currentEditionAammddBrt, // #3113 item 9: filtro de edição futura no arquivo
} from "./lib";
...
<style>
  ...
  p.sub { color: ${DS_COLORS.ink}; font-size: 0.9rem; }
 ...
 * durante a preparação de imagens/revisão, Etapa 4 — ver CLAUDE.md) — sem
 * este filtro, o arquivo expunha uma edição futura como votável antes da
 * newsletter ser publicada. `now` opcional (default `new Date()`) pra
...
export function extractEditionsForYear(correctKeyNames: string[], year: string, now: Date = new Date()): string[] {
  ...
  const today = currentEditionAammddBrt(now);
 ...
 * #3113 (item 10): agrupada por mês (heading + <ul> próprio) em vez de uma
...
<style>
  ...
  /* #3113 item 10: heading de mês — agrupa a lista flat que passaria de
     200 itens/ano. Reusa a convenção visual do .kicker (sans, uppercase,
  ...
  .month-heading:first-of-type { margin-top: 20px; }
${renderBrandShellStyles()}
</style>
</head>
<body>
...
<style>
  ...
  /* #3113 item 8: abaixo de 600px, o layout ANTERIOR empilhava as escolhas em
     largura total (flex-basis: 100%) — a imagem A + botão sozinhos já
     preenchiam a tela, permitindo votar em A sem nunca rolar até ver a
     imagem B. Fix na raiz: mantém as 2 escolhas lado a lado (cada uma
     dividindo o espaço disponível, mais estreitas) em vez de empilhar — as
     duas imagens ficam sempre visíveis ao mesmo tempo, sem depender de o
     leitor notar um hint nem de JS pra gatear o botão. */
...
export async function handleArchiveVotePage(
  ...
  if (!correctRaw || edition > currentEditionAammddBrt(new Date())) {
    return new Response(
