...
import { resolveSubscribeUtm, subscribeViaConfiguredBackend, JOGAR_IDENTIFY_MAGIC_LINK_REFERRING_SITE } from "./subscribe"; // #6291: função única, ramifica por SUBSCRIBE_BACKEND internamente
...
export async function handleConfirmMerge(
  ...
  if (pending.optin) {
    ...
    try {
      ...
      const result = await subscribeViaConfiguredBackend(bEnv, { name: pending.name, email: pending.email }, fetchImpl, utm);
      if (!result.ok) {
