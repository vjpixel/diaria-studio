 ...
 * Consumido por `renderOpenRateChartSvg` abaixo (o gráfico principal de
 * "Open rate por dia"). **#5876:** também alimentava `renderConfoundersSparklineSvg`
 * (A4, `padLeft`/`padRight`/`viewBoxW` pra alinhamento pixel-a-pixel em X),
 * removida por decisão editorial — `CHART` continua em uso, só perdeu esse
 * 2º consumidor.
 */
export const CHART = {
  viewBoxW: 1450,
...
// #5876 (21/08/2026): `renderCohortSmallMultipleSvg` (A2, small multiples
// por coorte), `renderConfoundersSparklineSvg` (A4, sparklines de
// bounce/spam/CTOR), `computeLagCorrelations`/`pearsonCorrelation` e
// `renderDeliveredOpenRateScatterSvg` (A3, dispersão delivered×openRate)
// foram implementados no #5640 e removidos aqui por decisão editorial —
// nenhum dos três estava entregando leitura acionável no estado real dos
// dados (ver docstring de `renderOpenRateByDaySection`, sections-core.ts,
// pro racional completo). Não reimplementar por engano — #5640 no
// histórico do git documenta o desenho original caso o editor peça de
// volta. `CHART` (usado pelo gráfico principal) e `niceMax`/
// `catmullRomToBezierPath`/`splitIntoRuns`/`enumerateDayRange` (usados por
// `renderOpenRateChartSvg` acima) continuam em uso — não eram exclusivos
