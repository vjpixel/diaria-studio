...
>
> **Decisão final (#2501, `/diaria-develop` 260701):** manter o caminho manual via Chrome — **sem upgrade de plano Beehiiv**. Segue o princípio de zero custo recorrente do CLAUDE.md; o editor já finaliza subject/preview/slug/capa na UI a cada publicação (fluxo estável, sem fricção adicional relevante para justificar assinatura paga). Reavaliar só se o volume de publicações crescer o suficiente para tornar o passo manual um gargalo real.
...
> e-mail 404 pra quem abrir. `scripts/check-whatsapp-slug-guard.ts` (§9) é o
> guard que confere isso e bloqueia o Stage 6 — mas conferir aqui, antes do
> paste, evita descobrir o problema só depois do Schedule.
...
**#3221 — mesmo risco no fluxo MANUAL, verificado automaticamente desde essa issue.** O caminho de publicação manual (`prep-manual-publish.ts`, ver §"Publicação manual" no `CLAUDE.md`) não usa o template "HTML" — reaproveita um post rascunho fixo chamado **"Default"** (`findDefaultTemplate()`), que sofre da mesma persistência de conteúdo entre edições que o #2283 documenta pro template "HTML". Caso concreto que motivou a issue: `scripts/lib/newsletter-render-html.ts` foi corrigido (linhagem #3220) pra remover o destaque visual (bold+uppercase+letter-spacing+ponto teal, decisão #3104) da linha "Resultado da última edição: X% acertaram." — o fix já é o novo default pra qualquer HTML gerado pelo pipeline, mas se o post "Default" tivesse sido salvo no Beehiiv com o htmlSnippet de uma edição anterior ao fix (ainda com o estilo antigo) e o editor não sobrescrevesse tudo antes do próximo paste, o estilo antigo reapareceria visualmente mesmo com o renderer já certo.

`prep-manual-publish.ts` agora roda **`checkTemplateNotStale`** como parte do gate de pré-condições (mesmo grupo de `checkNewsletterHtml`/`checkWorker`): busca o conteúdo persistido do post "Default" via API (`expand[]=free_web_content`, mesmo padrão de `fetch-beehiiv-poll-stats.ts`) e roda `hasStaleResultLineStyle()` (função pura, testada em `test/prep-manual-publish.test.ts`) — que procura um `<p style="...">Resultado da última edição` cujo `style` tenha `font-weight:bold` + `letter-spacing` + `text-transform:uppercase` juntos (a assinatura exata do estilo pré-#3220). Se detectar, o check falha e o script aborta com instrução pra limpar o Custom HTML block do template "Default" antes do paste. Falha de rede/API na checagem em si é fail-open (`passed: true` com aviso) — não trava o fluxo manual por indisponibilidade da própria verificação, só pelo conteúdo stale confirmado.

**Limitação conhecida (documentar, não é bug):** essa checagem detecta especificamente a REGRESSÃO de estilo da linha "Resultado da última edição" (o caso concreto da issue #3221) — não é uma verificação genérica de "o template não tem NADA de uma edição antiga". Se o editor quiser confirmar isso de forma mais ampla (conteúdo de destaques/imagens de uma edição anterior sobrevivendo no template), a verificação continua sendo manual: abrir o post "Default" no Beehiiv (Settings → Post Templates ou direto pela URL impressa por `prep-manual-publish.ts`) e conferir visualmente antes do paste, do mesmo jeito que #2283 já recomendava pro template "HTML".
...
**Causa raiz do #6608 resolvida (260828): o picker do "Search for a specific subscriber" (`input[placeholder="Search for a specific subscriber"]`) NÃO está quebrado — o debounce é só mais longo do que as tentativas anteriores esperaram.** Reproduzido ao vivo: clique real + `computer.type` no campo, seguido de espera, faz o dropdown renderizar normalmente; clique num item da lista popula o campo. **A latência varia com o tamanho da query** — query curta (`"vjpixel"`) resolveu em ~8s; query mais longa/específica (`"vjpixel@gmail"`) levou ~15s. Procedimento: digitar (parcial do e-mail já basta — queries curtas respondem mais rápido), esperar **pelo menos 10s antes da 1ª checagem**, e se ainda vazio insistir com esperas adicionais de 5s (`screenshot`/`find`) até ~20s totais antes de concluir vazio — nunca assumir vazio com menos de 10s de espera. Só depois de ~20s sem nenhum resultado é que vale suspeitar de regressão real da Beehiiv (reabrir #6608, citando a query usada e o tempo esperado). `--send-mode generic` (ver `review-test-email.md`) segue existindo como fallback pra quando não há Chrome disponível na sessão, não porque o picker esteja quebrado.
