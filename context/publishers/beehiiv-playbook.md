...
**Causa raiz do #6608 resolvida (260828): o picker do "Search for a specific subscriber" (`input[placeholder="Search for a specific subscriber"]`) NÃO está quebrado — o debounce é só mais longo do que as tentativas anteriores esperaram.** Reproduzido ao vivo: clique real + `computer.type` no campo, seguido de **espera de 8-10s** (não os ~2-4s testados no #6608 original), faz o dropdown renderizar normalmente; clique num item da lista popula o campo. Procedimento: digitar (parcial do e-mail já basta), esperar 8-10s, `screenshot`/`find` pra confirmar a lista renderizou antes de clicar — nunca assumir vazio antes desse intervalo. Se ainda assim não renderizar após 10s, aí sim é sinal de regressão real da Beehiiv (reabrir #6608). `--send-mode generic` (ver `review-test-email.md`) segue existindo como fallback pra quando não há Chrome disponível na sessão, não porque o picker esteja quebrado.
