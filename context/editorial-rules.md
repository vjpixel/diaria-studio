...
- **No máximo 2 URLs do mesmo domínio por edição (#5735).** Contagem sobre a edição INTEIRA (destaques D1/D2/D3 + RADAR + USE MELHOR + LANÇAMENTOS + vídeo somados, não por seção) — 2 links de `theverge.com` nos destaques já esgotam a cota, e nenhum item do pool pode linkar pro Verge naquela edição. Agrupamento por domínio registrável (eTLD+1): `blog.openai.com` e `openai.com` contam como o mesmo domínio. Sem allowlist de exceção pra plataformas (github.com, youtube.com, substack.com) — se virar problema real na prática, tratar em issue nova em vez de antecipar. Enforcement HARD (exit ≠0, não warning). Diversidade de fonte é qualidade editorial: várias URLs do mesmo veículo numa edição lê como agregador de um site só, não como curadoria. Validação: `npx tsx scripts/validate-domain-diversity.ts <md>` (helper de agrupamento em `scripts/lib/registrable-domain.ts`). Fora de escopo: digest mensal (`/diaria-mensal`) e concentração de domínio *entre* edições consecutivas.
...
- **Toda edição linka `arquivo.diar.ia.br` de duas formas combinadas, sem exceção (#5722, decisão do editor 19-20/08/2026).** Promover o acervo como superfície canônica de crawler (#5692/#5722) exige um caminho de entrada constante — resolvido A+B, nunca só um dos dois (A cobre alcance, B cobre relevância):
  - **(A) Rodapé fixo, incondicional.** A pill `[Edições anteriores]` no grupo "Da diar.ia.br:" de PARA ENCERRAR — bloco `CURADORIA_PILLS`, `scripts/lib/shared/encerramento-snippet.ts` — está presente em TODA edição, diária e mensal, sem flag/config que desative. Não depende de nenhum match temático; é o mesmo link em toda edição, sempre no mesmo lugar.
  - **(B) Box contextual, condicional a gancho temático real.** O grupo "Mais sobre {tema}" (1 hub + até 2 edições passadas do mesmo hub) é injetado no fim do corpo quando as manchetes do dia casam **exatamente 1** hub temático — `scripts/lib/related-editions.ts` (`selectRelatedEditions`/`renderRelatedEditionsMarkdown`, #5122/#5181), com exclusão mútua e dedup contra o bloco irmão "Saiba mais:" (`scripts/lib/hub-match.ts`, #4907, que injeta o mesmo tipo de link dentro do corpo do destaque quando o match é único edição-wide). 0 ou 2+ hubs casados → nenhum link (ambíguo, sem escolha implícita por ordem). **Ambos A e B já estavam implementados e testados antes da decisão do #5722** (`test/encerramento-social-apoio-3219.test.ts`, `test/hub-match.test.ts`, `test/related-editions-5122.test.ts`) — a decisão do editor confirmou reter os dois mecanismos como a resposta ao item 3 da issue, sem exigir código novo.
...
- **Não** mencionar "Noite Estrelada" ou obras reconhecíveis de Van Gogh — **nem em instrução negativa** (ex: "sem espirais estilo Noite Estrelada"): nomear a obra pra dizer "evite" tende a ancorar o modelo nela mesmo assim (#4201). Evitar céu noturno estrelado com pinceladas giratórias — produz clone de Noite Estrelada. Preferir linguagem **positiva** (descrever o estilo desejado — impasto denso, paleta quente, pinceladas grossas) em vez de negativa (listar o que evitar). `scripts/lint-image-prompt.ts` bloqueia o nome da obra e o motivo de "espiral"/"redemoinho" perto de céu/estrelas/amarelo — mas o objetivo é escrever o prompt já limpo, não depender do lint pra pegar.
...
- **Padrão de títulos: clickbait elegante (#6008, decisão do editor 260824).** Títulos escritos pra maximizar abertura/clique mantendo a linha editorial — sem mentira, sem inflar fato, sem vulgaridade. Táticas permitidas: (1) **tensão/drama factual** — verbos e framing de conflito ("trai", "engole", "quebra") quando o fato sustenta, sem inflar magnitude; (2) **pergunta provocativa** — título em forma de pergunta que o leitor quer respondida; (3) **referência direta ao leitor** — segunda pessoa / impacto pessoal. **Explicitamente proibido:** curiosity gap (reter informação pra forçar clique — "o detalhe que a OpenAI escondeu"): o título entrega a informação, o hook vem do framing, não da omissão. Também proibida a faixa vulgar: "você não vai acreditar", "chocante", exclamação, CAPS LOCK, reticências de suspense no fim, listicle vazio ("5 coisas que...") — flagrados pelo lint warning-only `title-clickbait-vulgar` no gate da Etapa 4. Vale pra títulos de destaque (diário), subject lines/títulos narrativos (mensal) e hooks de abertura social. O critério "ação concreta vs estimativa/cifra macro" (#5809) deixa de ser primário e vira **desempate** entre opções de tom equivalente.
- **Evitar "IA"/"inteligência artificial"/"AI" no título sempre que a frase continuar clara sem o termo (#4825).** Prefira nomear o agente concreto (empresa, modelo, produto) em vez da categoria — ver Seção 5 (Linguagem) pra regra completa, exceções legítimas e o lint warning-only que sinaliza ocorrências no gate da Etapa 4.
...
- **Evitar "IA"/"inteligência artificial"/"AI" sempre que possível — inclusive em títulos (#4825).** A newsletter é sobre IA — o contexto já está dado, e o termo raramente carrega informação nova. Prefira nomear o agente concreto: em vez de "a IA gerou X", escreva "o modelo gerou X"; em vez de "ferramentas de IA", escreva "ferramentas como X"; em vez de "Nova IA da Anthropic escreve código sozinha", escreva "Claude escreve código sozinha". **Exceções legítimas (não é proibição absoluta):** (1) a manchete é sobre a categoria em si, não sobre um produto específico (ex: "regulação de IA na UE"); (2) omitir o termo cria ambiguidade real sobre do que se fala; (3) o termo aparece em nome próprio, citação direta ou nome de produto (ex: "OpenAI", "Perplexity AI"). Lint warning-only (`lint-newsletter-md.ts --check title-mentions-ia`) sinaliza ocorrências em títulos de destaque no gate da Etapa 4, sem bloquear — a decisão final é sempre do editor.

- **Nunca "agentivo/agentiva/agentivos/agentivas" — sempre "agêntico/agêntica/agênticos/agênticas" (#7260).** "Agentivo" é termo de linguística (papel temático do agente numa oração); o termo do domínio de IA, correspondente a *agentic*, é "agêntico". Decisão permanente do editor (03/09/2026), depois de a forma errada reincidir em 7 edições ao longo de 4 meses (260510, 260515, 260518, 260625, 260731, 260821, 260903) apesar da forma certa já ser dominante no corpo de texto publicado. **Sem exceção legítima** — diferente da regra de "IA"/"AI" acima, "agentivo" nunca é a palavra certa num texto sobre sistemas que agem sozinhos, então o lint (`lint-newsletter-md.ts --check banned-lexicon` / `lint-social-md.ts --check banned-lexicon`, tabela de substituições em `scripts/lib/lint-checks/banned-lexicon.ts`) é **GATE-BLOCKING** na Etapa 4, sobre `02-reviewed.md` e `03-social.md` (só texto publicado — campos de raciocínio interno de LLM em JSON, ex. `01-categorized.json`, ficam fora do escopo de propósito).
