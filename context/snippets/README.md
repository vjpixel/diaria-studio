...
| `artigo-especial-apoiadores.md` | Slot 3 quando pinado por `/diaria-artigo-especial` (#5979) — também colável em 1/2 | multi-parágrafo, sem CTA pill (mesmo fallback do #3306) | Não é um dos defaults ESTÁTICOS de `boxes_divulgacao`, mas desde #5979 a skill `/diaria-artigo-especial` (Passo 5, `scripts/update-artigo-especial-box.ts`) escreve `boxes_divulgacao.slot3` + `boxes_divulgacao_auto.pinned_slots: [3]` automaticamente a cada artigo especial novo — não é mais preciso colar manualmente no draft (o mecanismo marcador-agnóstico de `apoio-divulgacao.md` segue funcionando pra reuso pontual fora da skill, se algum dia necessário). | Automático via `/diaria-artigo-especial`. Título do artigo em negrito inline (corrigido no gate 260724 — não usar "Mandei... há pouco", fixar "O Artigo Especial desse mês é: **título**"). A skill reescreve `{título do artigo}`/`{mês}`/gancho a cada artigo novo (edição cirúrgica — preserva header + parágrafo do tier + CTA); `--unpin` da skill devolve o slot 3 pro auto-select por cliques (#4626) quando o artigo envelhecer. |
