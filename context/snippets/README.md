...
| ~~**Slot 3** (pós-último-destaque, #3476)~~ **ELIMINADO (#6748, 29/08/2026)** | ~~Região entre o ÚLTIMO destaque (D3 em edições de 3, D2 em edições de 2) e USE MELHOR — existia em QUALQUER contagem de destaques, diferente do slot 2~~ | `boxes_divulgacao.slot3` continua no schema (back-compat/reversibilidade — decisão do PR #6748), mas `stitch-newsletter.ts` nunca mais lê esse valor e `select-boxes-by-clicks.ts` nunca mais o ranqueia/pina. `extractBoxDivulgacao3`/`locateBoxAfterLastDestaque` seguem existindo só para ler edições ARQUIVADAS de antes do #6748. | Ignorado — qualquer valor gravado ali não tem efeito. |
...
**#6748 (29/08/2026): a edição volta a ter no máximo 2 boxes de divulgação
(slot1+slot2), revertendo a decisão do #3476/#3824 abaixo que tornou o slot
3 permanente.** O texto histórico desta seção (default "3 boxes permanentes",
"apoio-divulgacao.md é o default de slot3") descreve o estado ENTRE #3824 e
#6748 — hoje `boxes_divulgacao.slot3` está `null` em `platform.config.json`
e `apoio-divulgacao.md` saiu da rotação (não migrou pro slot2, que já tinha
outro box ativo). Reintroduzir `apoio-divulgacao.md` num slot exige decisão
editorial nova.
...
"livros-divulgacao.md"` (curadoria geral de livros — rotaciona por cliques,
`boxes_divulgacao_auto`), `slot3: null` (eliminado, #6748).
...
no slot 2 — back-compat; slot 3 nunca entra nesse fallback, #6748).
...
o último destaque (D3 ou D2 — nunca desaparece, #1085; antes do #6748 o
fallback citava "o slot 3", que não existe mais) — ver `renderHTML` em
...
| `apoio-divulgacao.md` | Era slot 3 (default, #3824) — hoje **fora da rotação** (#6748, 29/08/2026: slot 3 eliminado; não migrado pro slot2, que já tem outro box ativo) — colável manualmente em slot 1/2 | multi-parágrafo com lista + CTA pill | Reintroduzir num slot exige decisão editorial nova. Colar manualmente no draft (`02-d1-draft.md`/`02-d2-draft.md`/`02-d3-draft.md`) continua funcionando pra rodar pontualmente — o parse do lado do render é marcador-agnóstico e não depende de `loadDivulgacaoSnippet`. | Programa de apoio (apoia.se/diaria) com lista de recompensas em bullets. |
| `artigo-especial-apoiadores.md` | Slot 2 quando pinado por `/diaria-artigo-especial` (#5979; era slot 3 até o #6748) — também colável em 1/2 | multi-parágrafo, sem CTA pill (mesmo fallback do #3306) | Não é um dos defaults ESTÁTICOS de `boxes_divulgacao`, mas desde #5979 a skill `/diaria-artigo-especial` (Passo 5, `scripts/update-artigo-especial-box.ts`) escreve `boxes_divulgacao.slot2` + `boxes_divulgacao_auto.pinned_slots: [2]` automaticamente a cada artigo especial novo (default mudou de slot3 para slot2 no #6748, já que slot3 nunca mais renderiza — trade-off aceito: em edição de 2 destaques o box não aparece, diferente do antigo slot3) — não é mais preciso colar manualmente no draft (o mecanismo marcador-agnóstico de `apoio-divulgacao.md` segue funcionando pra reuso pontual fora da skill, se algum dia necessário). | Automático via `/diaria-artigo-especial`. Título do artigo em negrito inline (corrigido no gate 260724 — não usar "Mandei... há pouco", fixar "O Artigo Especial desse mês é: **título**"). A skill reescreve `{título do artigo}`/`{mês}`/gancho a cada artigo novo (edição cirúrgica — preserva header + parágrafo do tier + CTA); `--unpin` da skill devolve o slot 2 pro auto-select por cliques (#4626) quando o artigo envelhecer. |
