...
- **UTM: o render injeta, o template quase nunca escreve.** `withClariceUtm` (#2975) sobrescreve `utm_source`/`utm_medium`/`utm_campaign` a cada render e codifica a POSIÇÃO do link no `utm_campaign` (#4040) — `cta`, `pill`, `titulo`, `inline` e, desde este CTA, `use-melhor`. Não adicionar `utm_*` à mão nos links da APRESENTAÇÃO nem do PARA ENCERRAR: eles são pinados por invariante (#3971/#2913) e o teste quebra.
  **Exceção — `utm_term=use-melhor` no CTA do Use Melhor.** O GA tracking da Brevo reescreve `utm_source`/`utm_medium`/`utm_campaign` no save do HTML (inclusive via API) mas **preserva `utm_term`** (descoberto no CTA-01, 260722). Como `withClariceUtm` usa `set` só nos três primeiros, o `utm_term` atravessa o render intacto — então o link sai marcado duas vezes e a medição sobrevive mesmo se alguém religar o toggle de tracking na UI. Esse link não é pinado por nenhum invariante, então a exceção não quebra nada.
