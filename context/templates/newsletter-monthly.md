...
- **Boxes de divulgação mid-body (colados manualmente entre destaques, isolados por `---` — não auto-inseridos pelo pipeline, decisão do editor 260716):** (a) **recomendação da diária pra base da Clarice** — fonte única `context/snippets/diaria-recomendacao-clarice.md` (tom de endosso "Recomendação da equipe da Clarice", CTA "Assinar grátis" com `utm_source=clarice`); (b) **box de livro** — usar `context/snippets/recomendacao-leitura-mensal.md` ("Recomendação de leitura", instância de referência: 2041/Kai-Fu Lee, texto **impessoal** com mini-bio do autor), **não** o `livros-divulgacao.md` genérico do diário nem o `recomendacao-leitura.md` (esse fica em 1ª pessoa, exclusivo do diário). Substituem os boxes improvisados "Um recado rápido da equipe da Clarice" e "LIVROS/Curadoria de livros" da edição 2606-07.
