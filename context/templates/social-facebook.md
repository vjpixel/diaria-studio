# Template — Post Facebook (diar.ia.br)

## Regras

- Abrir com **hook direto**: dado concreto ou fato surpreendente — sem pergunta retórica.
- Tom mais acessível que o LinkedIn: menos jargão técnico, mais ancoragem no cotidiano.
- 2–3 parágrafos curtos.
- **Call-to-action final em 1 frase**: `"Receba notícias de IA todo dia por e-mail, assine grátis em https://diar.ia.br."` — não mencionar o nome "diar.ia.br" no corpo (a URL carrega a identidade).
- Até 3 hashtags (menos é mais no Facebook).
- 800–1.200 caracteres (Facebook recompensa posts mais curtos).

## Estrutura

```
[Hook — 1 frase com dado ou fato concreto]

[Parágrafo 1 — desenvolve o fato em linguagem acessível]

[Parágrafo 2 — "por que isso muda alguma coisa" para o leitor comum]

Receba notícias de IA todo dia por e-mail, assine grátis em https://diar.ia.br.

#Hashtag1 #Hashtag2
```

## Diferença em relação ao LinkedIn

- Menos análise de mercado, mais impacto concreto na vida/trabalho.
- Frases mais curtas.
- Evitar termos como "benchmark", "deployment", "fine-tuning" sem explicação.

## Não fazer

- Não usar linguagem corporativa.
- Não usar mais de 3 hashtags.
- Não repetir o hook no parágrafo seguinte.
