...
**Revisão fora do terminal (mobile):** o Studio (`npm run studio`, acesso remoto via Cloudflare Tunnel desde #3560) cobre a revisão/edição dos outputs de cada etapa fora do terminal — inclusive no celular. O Google Drive sync que cumpria esse papel antes do Studio existir foi aposentado (#3636); os scripts `scripts/drive-sync.ts` e `scripts/oauth-setup.ts` continuam no repo (não removidos nesta limpeza — ver #3636 pra follow-up), mas nenhuma etapa do pipeline os invoca mais automaticamente.
