...
- **Etapa 4 (publicadores) default = tudo automático (#336 invertido em #1326).** Stage 4 é dispatch — editor já revisou nos gates 1-3. Default em modo interativo, em `auto_approve`, e em `--no-gates` é o mesmo: dispatchar nos 3 canais (newsletter Beehiiv via Worker-hosted, LinkedIn agendado 17:00 BRT, Facebook imediato). Editor pode opt-out por canal via flag `--skip {newsletter,linkedin,facebook}` ou via gate interativo (`/diaria-4-publicar` apresenta menu numérico). Se editor não responder no gate, fallback é tudo auto. Blast radius alto mas mitigado: Beehiiv sai como rascunho (com test email loop antes do schedule), LinkedIn agenda 24h+ à frente, Facebook é o único imediato — todos reversíveis por ação do editor no dashboard de cada plataforma. Logar source da decisão em `_internal/05-publish-consent.json` (`source: "default_auto" | "skip_flag_X" | "editor_response_X" | "auto_approve_default"`).
...
- **Pull antes de editar Reports Drive (extensão universal de #494, instituída 2026-05-18).** Vale também pra `data/reports/*.md` (status-parceria, aprendizados, relatorio-envios). Antes de **qualquer** Edit/Write em report sincronizado, rodar `npx tsx scripts/fetch-drive-docs.ts {file_id}` + copiar `/c/tmp/drive-docs/{file_id}.md` (strip dos 3 primeiros lines de metadata) → `data/reports/{name}.md`. Ou usar `sync-report.ts --dry-run` pra diff. **Force-push sem pull = sobrescreve silenciosamente edições do editor.** Incidente 2026-05-18: editor reportou que 5+ rodadas de edits manuais (paragraph splits, typo fixes "Açõess→Decisões", `SCORE` em backticks, restaurações de rows) foram perdidas em pushes sequenciais. Reconstrução demorou ~30min por revisão Drive. Se `sync-report.ts` falhar com conflito por causa de escape chars do Google Docs MD export, inspecionar diff manual antes de force-push, NUNCA force-push direto.
...
- **Verificar emails no MillionVerifier antes de enviar tier ≥ T02 (#1297).** Antes do primeiro envio de qualquer wave que inclua contatos não-ativos (ex-assinantes / leads — `verify_risk ≥ 3`), rodar `npx tsx scripts/verify-emails-mv.ts --input brevo-import-t{NN}.csv`. O script verifica cada email via MillionVerifier (resumível via checkpoint `.mv-cache-*.json` — re-rodar não re-gasta crédito), e divide em `{base}-verified.csv` (`ok`+`catch_all` → mandar pro Brevo), `{base}-rejected.csv` (`invalid`+`disposable` → excluir) e `{base}-unknown.csv` (inconclusivo). Importar **só o `-verified.csv`** no Brevo. Sem isso, bounce de 5–10% no T02 degrada a reputação do IP/domínio e contamina o T01 no mesmo IP. T01 (assinantes ativos, `verify_risk 1`) pula a verificação — pagamento Stripe já valida implicitamente. Requer `MILLION_VERIFIER_API_KEY` no env (custo one-time ~$1.90/1000).
...
- **Opus 4.6** — `scorer` (single-call legado / fallback), `scorer-chunk` + `scorer-select` (chunked-parallel, #1611)
