...
- **Publicação manual requer inject-poll-urls antes (#1044).** Sempre que for publicar manualmente no Beehiiv, **antes** de clicar Send, rodar `npx tsx scripts/inject-poll-urls.ts --edition AAMMDD` pra popular custom fields `poll_a_url`/`poll_b_url` com URLs HMAC pra essa edição. Sem isso, o template renderiza URLs da edição anterior (custom fields stale) e leitor que clicar em A/B vota na edição errada — pior que zero votos, atribui silenciosamente. Após publicar, rodar `npx tsx scripts/close-poll.ts --edition AAMMDD` pra registrar o gabarito no Worker.
