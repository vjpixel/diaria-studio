...
2. Alternativamente, rodar etapas isoladas:
   ...
   - **Etapa 5** (publicação): `/diaria-5-publicar [all|newsletter|social]` (ou `/diaria-4-publicar` como alias retrocompat).
...
- **Data da edição é sempre explícita.** Skills `/diaria-*` que aceitam `AAMMDD` **nunca** inferem a partir de `today()` ou da edição mais recente em `data/editions/`. Se o usuário não passar a data, perguntar com sugestão de hoje/ontem como atalho mas exigir confirmação. Risco de rodar stage destrutivo/publicador (Stage 5) na edição errada é alto demais pra default silencioso.
  ...
  Exceção (#583): skills `/diaria-2-escrita`, `/diaria-3-imagens`, `/diaria-4-revisao`, `/diaria-5-publicar` aceitam AAMMDD opcional. Se omitido, rodar `npx tsx scripts/lib/find-current-edition.ts --stage N` e — se exatamente 1 edição estiver em curso (prereq do stage atendido + output faltando) — assumir essa edição com info log. Caso 0 candidatos: erro. Caso ≥2: perguntar como antes. Stage 1 não muda — cria a edição.
...
- **Etapa 5 (publicadores) default = tudo automático (#336 invertido em #1326, #1694).** Stage 5 é dispatch — editor já revisou no gate do Stage 4 (Revisão). Default em modo interativo, em `auto_approve`, e em `--no-gates` é o mesmo: dispatchar nos 3 canais (newsletter Beehiiv via Worker-hosted, LinkedIn agendado 17:00 BRT, Facebook imediato). Editor pode opt-out por canal via flag `--skip {newsletter,linkedin,facebook}` ou via gate interativo (`/diaria-5-publicar` apresenta menu numérico). Se editor não responder no gate, fallback é tudo auto. Blast radius alto mas mitigado: Beehiiv sai como rascunho (com test email loop antes do schedule), LinkedIn agenda 24h+ à frente, Facebook é o único imediato — todos reversíveis por ação do editor no dashboard de cada plataforma. Logar source da decisão em `_internal/05-publish-consent.json` (`source: "default_auto" | "skip_flag_X" | "editor_response_X" | "auto_approve_default"`).
...
- **Publicação manual requer refresh-dedup.** Sempre que uma edição for publicada manualmente no Beehiiv (sem `/diaria-5-publicar`), rodar `/diaria-refresh-dedup` imediatamente após para manter `data/past-editions.md` atualizado. Sem isso, a próxima edição pode repetir URLs já publicadas.
...
- **Verificar emails no MillionVerifier antes de enviar tier ≥ T02 (#1297).** Antes do primeiro envio de qualquer wave que inclua contatos não-ativos (ex-assinantes / leads — `verify_risk ≥ 3`), rodar `npx tsx scripts/verify-emails-mv.ts --cycle {conteúdo}-{envio} --input stripe-export-t02-ex-assinantes.csv` (ex: `--cycle 2605-06`; pra outro tier, use o nome descritivo gerado pela merge — `ls data/clarice-subscribers/stripe-export-t*`). O input é a base (tier no root, **nome descritivo** `t{NN}-{slug}.csv`); as saídas verificadas vão pra subpasta do ciclo `data/clarice-subscribers/{conteúdo}-{envio}/` (#1961 — `{conteúdo}` = mês do digest, `{envio}` = mês do disparo, que é o seguinte). O script verifica cada email via MillionVerifier (resumível via checkpoint `.mv-cache-*.json` — re-rodar não re-gasta crédito), e divide em verified (`ok`+`catch_all` → mandar pro Brevo), rejected (`invalid`+`disposable` → excluir) e unknown (inconclusivo). **Proveniência no nome:** as saídas trocam o prefixo `stripe-export-` → `mv-export-` (são output do MV) — ex: `mv-export-t02-ex-assinantes-verified.csv`. Importar **só o `-verified.csv`** no Brevo. Sem isso, bounce de 5–10% no T02 degrada a reputação do IP/domínio e contamina o T01 no mesmo IP. T01 (assinantes ativos, `verify_risk 1`) pula a verificação — pagamento Stripe já valida implicitamente. Requer `MILLION_VERIFIER_API_KEY` no env (custo one-time ~$1.90/1000). **Os 3 scripts do ciclo exigem `--cycle {conteúdo}-{envio}`** (#1961): `verify-emails-mv` (escreve `mv-export-t02-ex-assinantes-verified.csv` no `{ciclo}/`), depois `clarice-build-waves --cycle ...` (lê T1 da base no root + `mv-export-t02-ex-assinantes-verified.csv` do `{ciclo}/`, escreve waves `wX-{ferramenta}-{tier}` em `{ciclo}/waves/` — W5 maio é opcional), depois `clarice-import-waves --cycle ...` (lê `{ciclo}/waves/`). O `--cycle` é validado (formato + envio = conteúdo+1) — typo de mês aborta limpo.
...
- **Opus 4.8** (#1951) — `orchestrator`, `scorer` (single-call legado / fallback), `scorer-chunk` + `scorer-select` (chunked-parallel, #1611), `analyst-monthly` + `scorer-monthly` (pipeline mensal). `title-picker` usa shorthand `opus` (auto-tracks latest = 4.8). Decisão editorial pós-gate, volume baixo, alto impacto em CTR — #159.
