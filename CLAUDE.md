...
- **Risco aceito: sem detecção de conflito editor(Studio)×pipeline em `02-reviewed.md`/`03-social.md` (#3729).** A retirada do Drive sync (#3636) removeu junto o único mecanismo que existia pra esse cenário — o push do Stage 2 rodava com `--on-conflict pull-merge --fail-on-conflict` (3-way merge + halt banner quando editor e pipeline tocavam a mesma região). Hoje os dois lados escrevem DIRETO no arquivo local compartilhado sem CAS nem lock: se o editor salvar no Studio (`saveReviewFile` em `scripts/studio-ui/studio-review.ts` — escrita direta do conteúdo inteiro, por design, ver comentário no topo do arquivo) no exato momento em que o pipeline reescreve o mesmo arquivo pós-gate (title-picker, Clarice, humanizador), um dos dois lados perde a edição silenciosamente. **Decisão consciente, não descuido:** investigado no #3729 (rodada overnight 260719/20) — um lockfile análogo ao já usado em `scripts/lib/social-published-store.ts` não é viável aqui porque o lado pipeline escreve via `Edit`/`Write` do agente (LLM tool call), não via script interceptável que possa checar/segurar um lock; construir esse mecanismo do zero é decisão de arquitetura, não fix mecânico — não implementado sem supervisão do editor nesta rodada overnight (critério de escopo do próprio #3729, não uma regra fixada em `context/overnight-dispatch-rules.md`). Mitigante parcial: Studio é tempo-real (janela de conflito é só a duração de uma escrita do pipeline, segundos) vs. o Drive assíncrono antigo (janela de horas); risco residual é baixo mas não-zero. Se o editor notar perda de edição nesse cenário, reportar no #3729 — é candidato a `/diaria-develop` (decisão de produto: aceitar como está / lock leve / warn-before-save no client do Studio reusando o guard de divergência que já existe pro slug `html-final`, #3635).
