...
2. Alternativamente, rodar stages isolados:
   - **Fase 1** (textos): `/diaria-research` (também refresca dedup + drena inbox), `/diaria-escrever`, `/diaria-social`.
   - **Fase 2** (imagens): `/diaria-eai` (Stage 4), `/diaria-imagens [d1|d2|d3]` (Stage 5).
   - **Fase 3** (publicação): `/diaria-publicar [newsletter|social|all]` (Stages 6 + 7).
...
- **Haiku 4.5** — todos os outros (`source-researcher`, `discovery-searcher`, `link-verifier`, `deduplicator`, `categorizer`, `clarice-runner`, `social-*`, `eai-composer`, `image-prompter`)
