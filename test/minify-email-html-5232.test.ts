...
// ── Integração CLI real (produção) — #5290 self-review finding 1 ──────────
//
// Os testes acima cobrem `minifyEmailHtml()`/`renderHTML()` isolados. NENHUM
// deles roda o binário `scripts/render-newsletter-html.ts` de fato — o
// caminho real de produção, invocado como
// `npx tsx scripts/render-newsletter-html.ts {EDITION_DIR}/ --format html
// --out {EDITION_DIR}/_internal/newsletter-draft.html`
// (ver `.claude/agents/orchestrator-stage-4.md`). Se um refactor futuro
// remover o wrapper `minifyEmailHtml(...)` de algum dos write points do CLI
// (`bodyHtml`, `minifiedEiaHtml`, `output`), nenhum teste acima pegaria a
// regressão — todos chamam as funções puras diretamente, nunca o arquivo que
// o CLI de fato grava em disco. Padrão de subprocess (spawnSync sobre o CLI
// real, dir de fixture temporário) espelha
// test/render-newsletter-html-link-layout-4841.test.ts.

const PROJECT_ROOT = resolve(import.meta.dirname, "..");
const SCRIPT = join(PROJECT_ROOT, "scripts", "render-newsletter-html.ts");

/** Fixture mínima com 1 destaque — o suficiente pra exercitar o comentário
 * de debug `<!-- Destaque N -->` (scripts/lib/newsletter-render-html.ts) e o
 * whitespace estrutural (newline + indentação) entre as tags que o template
 * emite entre `parts` (ver `renderHTML`). */
function makeEditionDir(): string {
  const base = mkdtempSync(join(tmpdir(), "diaria-minify-cli-"));
...
function runCli(dirAndArgs: string[]) {
...
describe("render-newsletter-html CLI — HTML escrito em disco sai minificado (#5290 self-review finding 1)", () => {
  it("--format html --out (invocação real de produção, Stage 4): arquivo gravado não tem comentário de debug nem whitespace estrutural", () => {
    const dir = makeEditionDir();
    try {
      const outPath = join(dir, "_internal", "newsletter-draft.html");
      const r = runCli([dir, "--format", "html", "--out", outPath]);
      assert.equal(r.status, 0, `stderr: ${r.stderr}`);

      const written = readFileSync(outPath, "utf8");

      // Comentário de debug do gerador (`<!-- Destaque N -->`,
      // `<!-- diar.ia.br newsletter body — auto-generated ... -->`) nunca
      // deveria sobreviver no arquivo GRAVADO — só `minifyEmailHtml()`
      // remove isso; se o wrapper cair do write point, este assert falha.
      assert.doesNotMatch(written, /<!-- Destaque/);
      assert.doesNotMatch(written, /auto-generated by render-newsletter-html/);

      // Whitespace estrutural (newline + indentação entre 2 tags) também só
      // colapsa via `minifyEmailHtml()` — sinal direto de que a minificação
      // rodou sobre o HTML que de fato foi pro disco, não só em memória.
      assert.doesNotMatch(written, />\s*\n\s*</);

      // Comentário condicional MSO precisa sobreviver (não é debug — é
      // markup real interpretado pelo Outlook, #260629b).
      assert.match(written, /<!--\[if mso\]>/);
      assert.match(written, /<!\[endif\]-->/);

      // Conteúdo editorial visível não pode ter sumido junto.
      assert.ok(written.includes("Título um"));
      assert.ok(written.includes("https://example.com/d1"));
    } finally {
      rmSync(resolve(dir, ".."), { recursive: true, force: true });
    }
  });

  it("--split: os 2 artefatos gravados (newsletter-body.html + newsletter-eia.html) saem minificados", () => {
    const dir = makeEditionDir();
    try {
      const r = runCli([dir, "--split"]);
      assert.equal(r.status, 0, `stderr: ${r.stderr}`);

      const bodyPath = join(dir, "_internal", "newsletter-body.html");
      const body = readFileSync(bodyPath, "utf8");
      assert.doesNotMatch(body, /<!-- Destaque/);
      assert.doesNotMatch(body, />\s*\n\s*</);
      assert.ok(body.includes("Título um"));
    } finally {
      rmSync(resolve(dir, ".."), { recursive: true, force: true });
    }
  });
