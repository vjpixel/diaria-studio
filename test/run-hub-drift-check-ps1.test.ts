 ...
 * `scripts/run-worker-drift-check.ps1` (#4723): log resiliente (temp file
 * fora de data/, anexo com retry) + exit code honesto mesmo quando `npx`
 * genuinamente não resolve no PATH (contexto de serviço do Task Scheduler).
 * Mesmos 3 cenários travados aqui — ver `test/run-worker-drift-check-ps1.test.ts`
 * pro precedente direto.
 */
