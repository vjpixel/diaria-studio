/**
 * test/run-cursos-error-alarm-ps1.test.ts (#4320)
 *
 * `scripts/run-cursos-error-alarm.ps1` segue o MESMO molde de
 * `scripts/run-clarice-guardrail-alarm.ps1` (#4064/#4343): log resiliente
 ...
 * Task Scheduler). Mesmos 3 cenários travados aqui:
...
const SCRIPT = join(ROOT, "scripts", "run-cursos-error-alarm.ps1");
...
describe(
  "run-cursos-error-alarm.ps1: log resiliente + exit code honesto (#4320)",
  { skip: !isWindows && "requer powershell.exe (Windows)" },
  () => {
    let workDir: string;

    before(() => {
      workDir = mkdtempSync(join(tmpdir(), "cursos-error-alarm-test-"));
    });
    ...
    it("log não gravável (parent do LogPath é um arquivo) -> exit != 0", () => {
      ...
      const badLogPath = join(blockerFile, "sub", ".error-alarm.log");
