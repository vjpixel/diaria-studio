...
describe("ds-golden-full-render (#2108) — golden de página inteira do renderHTML", () => {
  ...
  it("output não-vazio e tem estrutura de container email", () => {
    ...
    assert.ok(html.includes("max-width:656px"), "cap de largura do container ausente (#5176 — era 600px)");
    assert.ok(html.includes('class="container" width="100%"'), "container responsivo (width 100%) ausente");
    // #260629 (b): wrapper MSO cap no Outlook (ignora max-width). #5176: 600 → 656.
    assert.ok(html.includes('<!--[if mso]><table role="presentation" align="center" width="656"'), "wrapper MSO 656 (Outlook cap) ausente");
