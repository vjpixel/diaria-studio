...
describe("validateSubscribeInput (#3580)", () => {
  it("honeypot preenchido → status 200 error honeypot (descarte silencioso)", () => {
    const v = validateSubscribeInput({ name: "", email: "a@b.com", optin: true, honeypot: "bot", source: "", utmSource: "", utmMedium: "", utmCampaign: "", referrer: "", clickId: "" });
  ...
  it("opt-in NÃO marcado → 400 optin_required (consentimento LGPD obrigatório)", () => {
    const v = validateSubscribeInput({ name: "Ana", email: "a@b.com", optin: false, honeypot: "", source: "", utmSource: "", utmMedium: "", utmCampaign: "", referrer: "", clickId: "" });
    assert.deepEqual(v, { ok: false, status: 400, error: "optin_required" });
  });

  it("e-mail inválido → 400 invalid_email", () => {
    const v = validateSubscribeInput({ name: "Ana", email: "not-an-email", optin: true, honeypot: "", source: "", utmSource: "", utmMedium: "", utmCampaign: "", referrer: "", clickId: "" });
    assert.deepEqual(v, { ok: false, status: 400, error: "invalid_email" });
  });

  it("válido → ok com nome trimado e cortado em 100 chars", () => {
    const longName = "x".repeat(200);
    const v = validateSubscribeInput({ name: `  ${longName}  `, email: " a@b.com ", optin: true, honeypot: "", source: "", utmSource: "", utmMedium: "", utmCampaign: "", referrer: "", clickId: "" });
