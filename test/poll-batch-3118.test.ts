...
describe("maskEmail — consolidação das 3 implementações (#3118 item 11)", () => {
  it("email normal (local-part > 5 chars) → 5 primeiros chars + '…@***' (#4253 item 5)", () => {
    assert.equal(maskEmail("usuario@example.com"), "usuar…@***");
  });
  it("local-part com exatamente 5 chars → sem truncar (nada além dos 5 chars a esconder)", () => {
    assert.equal(maskEmail("abcde@example.com"), "abcde@***");
  });
  it("local-part com menos de 5 chars → sem truncar", () => {
    assert.equal(maskEmail("ab@example.com"), "ab@***");
  });
  it("email sem @ (defensivo — dado histórico pré-#3118 item 3) → mesma truncagem de 5 chars, sem '@'", () => {
    assert.equal(maskEmail("semarroba"), "semar…***");
  });
