/**
 * test/run-clarice-opens-catchup-alarm-ps1.test.ts (#4756)
 *
 * `scripts/run-clarice-opens-catchup-alarm.ps1` não tinha teste de
 * regressão dedicado até a extração de
 ...
 * (`run-cursos-error-alarm.ps1`/`run-clarice-guardrail-alarm.ps1`,
 * #4064/#4320/#4343). Cenários travados aqui:
...
const SCRIPT = join(ROOT, "scripts", "run-clarice-opens-catchup-alarm.ps1");
...
describe(
  "run-clarice-opens-catchup-alarm.ps1: log resiliente + exit code honesto (#4756)",
  { skip: !isWindows && "requer powershell.exe (Windows)" },
  () => {
    let workDir: string;

    before(() => {
      workDir = mkdtempSync(join(tmpdir(), "clarice-opens-catchup-alarm-test-"));
    });
    ...
    it("log não gravável (parent do LogPath é um arquivo) -> exit != 0", () => {
      ...
      const badLogPath = join(blockerFile, "sub", ".opens-catchup-alarm.log");
