...
// Usa o mês REAL corrente (recordBraveCreditEstimate grava com timestamp now) →
// as entradas seedadas precisam estar no mesmo mês pra computeBraveCreditStats contar.
...
test("reconcile: grava o gap do header como Path B; local passa a bater com o real", () => {
  ...
  main(["--edition", "260701"], path);

  const s = computeBraveCreditStats("260701", path);
...
test("reconcile: idempotente — re-rodar não duplica", () => {
  ...
  main(["--edition", "260701"], path); // gap = 2000-1990-1 = 9 → grava 9
  main(["--edition", "260701"], path); // idempotente → não grava de novo
  const s = computeBraveCreditStats("260701", path);
