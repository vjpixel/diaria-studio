...
describe("#7678 — seção '## eia' removida (vestigial, publish-eia-social.ts usa 01-eia-social.md)", () => {
  it("03-social.md NUNCA recebe '## eia' mesmo com 01-eia.md presente (regressão)", () => {
    const dir = makeEditionDir();
    try {
      writeFileSync(join(dir, "_internal", "03-social.tmp.md"), "## d1\n\nD1\n\n## d2\n\nD2\n\n## d3\n\nD3\n\n# LinkedIn\n\n## d1\n\nD1\n\n## post_pixel\n\nPixel.\n");
      ...
      assert.equal(r.status, 0, `stderr: ${r.stderr}`);
      const out = readFileSync(join(dir, "03-social.md"), "utf8");
      assert.ok(!out.includes("## eia"), "03-social.md deve NÃO conter '## eia' (#7678)");
      assert.ok(!out.includes("01-eia-A.jpg"), "nome de arquivo de bastidor não deve vazar (#7678)");
    } finally {
      rmSync(dir, { recursive: true });
    }
  });
