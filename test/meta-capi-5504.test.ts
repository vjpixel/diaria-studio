...
describe("#7776 — buildMetaCapiLogEvent (distingue not_configured de configurado-e-falhou)", () => {
  ...
  it("eventSourceUrl opcional repassado em todos os casos (#8647)", () => {
    ...
    assert.deepEqual(buildMetaCapiLogEvent({ ok: false, status: 401, reason: "meta_error" }, "reativar", undefined), { event: "meta_capi_send_failed", worker: "reativar", status: 401, reason: "meta_error", eventSourceUrl: undefined });
  });
