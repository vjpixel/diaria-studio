...
describe("check-invariants — checkNoBeehiivHostInPublicCta (#4059)", () => {
  ...
  // Header que o `wrangler types` escreve no topo do arquivo gerado — é ele que
  // habilita o skip, não o nome do arquivo sozinho.
  const WRANGLER_HEADER =
    "/* eslint-disable */\n" +
    "// Generated by Wrangler by running `wrangler types` (hash: 2b93fc5a)\n";
  const ENV_DECL =
  ...
  it("REGRESSÃO: worker-configuration.d.ts (gerado por `wrangler types`) não é flagado", () => {
    ...
    // que JÁ está na allowlist. Um worker novo com ALLOWED_ORIGINS não pode
    // voltar a reprovar o `--static`.
  ...
  it("o skip exige o header do wrangler — homônimo escrito à mão CONTINUA policiado", () => {
    // Sem esta condição, `worker-configuration.d.ts` viraria um nome de arquivo
    // permanentemente cego pro guard: bastaria commitar um arquivo com esse nome
    // (`git add -f`, cópia de backup, merge acidental) pra esconder um CTA sujo
    // até do CI, que aí passaria a ver o arquivo. Review da PR #4204.
    const dir = mkdtempSync(join(tmpdir(), "inv-cta-forjado-"));
    try {
      mkdirSync(join(dir, "workers", "novo"), { recursive: true });
      writeFileSync(
        join(dir, "workers", "novo", "worker-configuration.d.ts"),
        ENV_DECL, // mesmo nome, mesmo conteúdo — mas SEM o header do wrangler
        "utf8",
      );
      const violations = checkNoBeehiivHostInPublicCta(dir);
      assert.equal(violations.length, 1, JSON.stringify(violations, null, 2));
